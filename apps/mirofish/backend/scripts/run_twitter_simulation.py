...
Reads parameters from a configuration file to run the simulation fully automatically.

Features:
- After the simulation completes, the environment is kept alive in command-waiting mode.
- Supports receiving interview commands over IPC.
- Supports single-agent and batch-agent interviews.
- Supports remote close-environment commands.

Usage:
    python run_twitter_simulation.py --config /path/to/simulation_config.json
    python run_twitter_simulation.py --config /path/to/simulation_config.json --no-wait  # close immediately after completing
"""
...
class MaxTokensWarningFilter(logging.Filter):
    """Filters out camel-ai warnings about max_tokens (we intentionally omit max_tokens and let the model decide)."""

    def filter(self, record):
        # Filter out log records containing a max_tokens warning
        if "max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage():
            return False
        return True


# Add the filter immediately at module load time, so it takes effect before any camel code runs
logging.getLogger().addFilter(MaxTokensWarningFilter())


def setup_oasis_logging(log_dir: str):
    """Configure OASIS logging, writing to fixed-name log files."""
...
class IPCHandler:
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        ...
        # Collect command files (sorted by modification time)
        command_files = []
        ...
        try:
            ...
            for interview in interviews:
                ...
                try:
                    ...
                    print(f"  Warning: cannot retrieve agent {agent_id}: {e}")

            if not actions:
                self.send_response(command_id, "failed", error="No valid agents found")
                return False

            # Execute the batch interview
            await self.env.step(actions)
    ...
    def _get_interview_result(self, agent_id: int) -> Dict[str, Any]:
        """Retrieve the latest interview result for an agent from the database."""
...
class TwitterSimulationRunner:
    ...
    # Available Twitter actions (INTERVIEW is excluded; it can only be triggered manually via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: Whether to enter command-waiting mode after the simulation completes (default True)
        """
    ...
    def _get_profile_path(self) -> str:
        """Return the path to the profile file (OASIS Twitter uses CSV format)."""
    ...
    def _create_model(self):
        ...
        # Fall back to config if .env has no value
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        """
        Determine which agents to activate for the current round based on time and configuration.
        ...
        for cfg in agent_configs:
            ...
            # Skip if the agent is outside its active hours
            if current_hour not in active_hours:
                continue

            # Activate based on the agent's activity level
    ...
    async def run(self, max_rounds: int = None):
        ...
        Args:
            max_rounds: Maximum number of simulation rounds (optional; used to cap very long simulations)
        """
        print("=" * 60)
        ...
        # Cap at max_rounds if specified
        ...
        self.env = oasis.make(
            ...
            semaphore=30,  # Limit the maximum number of concurrent LLM requests to avoid API overload
        )
        ...
        for round_num in range(total_rounds):
            ...
            # Get the agents active for this round
        ...
        # Enter command-waiting mode if requested
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command-waiting mode — environment is still running")
            print("Supported commands: interview, batch_interview, close_env")
            print("=" * 60)

            self.ipc_handler.update_status("alive")

            # Command-waiting loop (uses the global _shutdown_event)
            try:
...
async def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to the configuration file (simulation_config.json)'
    )
    parser.add_argument(
        '--max-rounds',
        type=int,
        default=None,
        help='Maximum number of simulation rounds (optional; caps very long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately after the simulation completes, without entering command-waiting mode'
    )
    ...
    # Initialize logging (fixed filenames, clean up old logs)
...
def setup_signal_handlers():
    ...
    Gives the program a chance to release resources gracefully (close database, environment, etc.).
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit only if the signal is received again
            print("Forced exit...")
            sys.exit(1)
