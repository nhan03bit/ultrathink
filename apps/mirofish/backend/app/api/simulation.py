...
# Adding this prefix prevents Agents from calling tools and forces a direct text reply
INTERVIEW_PROMPT_PREFIX = "Drawing on your persona, all past memories, and past actions, reply to me directly in text without calling any tools: "
...
def get_graph_entities(graph_id: str):
    """
    Retrieve all entities in the graph (filtered).

    Returns only nodes that match predefined entity types (nodes whose Labels are not solely "Entity").
...
def get_entities_by_type(graph_id: str, entity_type: str):
    """Retrieve all entities of a specified type"""
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional; if not provided, taken from the project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required file list (scripts are not included; they reside in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # If config_generated=True and all files exist, consider preparation complete.
        # The following statuses all indicate that preparation has been completed:
        # - ready: preparation complete, ready to run
        # - preparing: if config_generated=True, preparation is already done
        # - running: simulation is running, so preparation was done earlier
        # - completed: simulation finished, so preparation was done earlier
        # - stopped: simulation stopped, so preparation was done earlier
        # - failed: simulation failed (but preparation was complete)
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status is "preparing" but all files are complete, automatically update the status to "ready"
            if status == "preparing":
                try:
                    ...
                    logger.warning(f"Auto status update failed: {e}")
...
def prepare_simulation():
    """
    Prepare the simulation environment (asynchronous task — all parameters are intelligently generated by the LLM).
    ...
    - If preparation is already complete, returns the existing result immediately
    - Supports forced re-generation (force_regenerate=true)
    ...
    4. Intelligently generate the simulation configuration using the LLM (with retry logic)
    ...
    Request (JSON):
        {
            ...
            "parallel_profile_count": 5,                  // Optional — number of parallel profile workers, default 5
            "force_regenerate": false                     // Optional — force re-generation, default false
        }
    ...
    try:
        ...
        # Check whether force re-generation is requested
        ...
        # Retrieve required information from the project
        ...
        # Retrieve the simulation requirement
        ...
        # ========== Synchronously retrieve entity count (before launching the background task) ==========
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Quick read of entities (no edge information needed; counting only)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Skip edge information to speed up the request
            )
            # Save entity count to state (so the frontend can access it immediately)
            ...
            # Failure does not block the subsequent flow; the background task will retry
        ...
        # Update the simulation status (includes the pre-fetched entity count)
...
def get_prepare_status():
    ...
    2. Check whether preparation has already been completed by simulation_id
    ...
    try:
        ...
        if not task_id:
            if simulation_id:
                # simulation_id provided but preparation is not complete
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # Task does not exist; if simulation_id is available, check whether preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Retrieve the latest report_id associated with a simulation.

    Iterates over the reports directory to find reports whose simulation_id matches;
    if multiple are found, returns the most recent one (sorted by created_at).
    ...
    # Path to the reports directory: backend/uploads/reports
    # __file__ is app/api/simulation.py; go up two levels to reach backend/
...
def get_simulation_history():
    """
    Retrieve the list of historical simulations (with project details).

    Used for the home page's historical project display. Returns a rich simulation list including
    project names, descriptions, and more.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Retrieve simulation configuration (reads simulation_requirement from simulation_config.json)
            ...
            # Retrieve run state (reads the user-configured total rounds from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use the user-configured total_rounds; fall back to the recommended value if absent
            ...
            # Retrieve up to 3 files from the associated project
            ...
            # Retrieve the associated report_id (find the most recent report for this simulation)
        ...
        logger.error(f"Failed to fetch simulation history: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles(simulation_id: str):
    """
    Retrieve the Agent Profiles for a simulation.
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Retrieve Agent Profiles in real time (for monitoring progress during generation).
    ...
    Response:
        {
            ...
            "data": {
                ...
                "is_generating": true,  // Whether generation is currently in progress
                "file_exists": true,
    ...
    try:
        ...
        # Check whether generation is in progress (by inspecting state.json)
        is_generating = False
        total_expected = None
        ...
        logger.error(f"Failed to fetch profiles in real time: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Retrieve simulation configuration in real time (for monitoring progress during generation).

    Differences from the /config endpoint:
    - Reads files directly, bypassing SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g. file modification time, whether generation is in progress)
    - Can return partial information even before generation is complete
    ...
    try:
        ...
        if os.path.exists(state_file):
            try:
                with open(state_file, 'r', encoding='utf-8') as f:
                    ...
                    # Determine the current generation stage
                    if is_generating:
        ...
        # If the config exists, extract key summary statistics
        if config:
        ...
        logger.error(f"Failed to fetch config in real time: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    """
    Retrieve the simulation configuration (full configuration intelligently generated by the LLM).
    ...
    Returns:
        ...
        - generation_reasoning: LLM reasoning notes for the generated configuration
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download a simulation run script (shared scripts located in backend/scripts/).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // Optional: maximum number of simulation rounds (truncates overly long simulations)
            "enable_graph_memory_update": false,   // Optional: whether to dynamically push Agent activity to the Zep graph memory
            "force": false                         // Optional: force restart (stops any running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, if the simulation is running or has already completed, it will be stopped first and run logs cleared
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities during the simulation (posts, comments, likes, etc.) are pushed to the Zep graph in real time
        ...
        - Uses batch updates to minimize API call count
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete; check whether there is a running process
                if state.status == SimulationStatus.RUNNING:
                    # Check whether the simulation process is actually still running
                ...
                # In force mode, clean up run logs
                if force:
        ...
        # Retrieve the graph ID (for graph memory update)
        graph_id = None
        if enable_graph_memory_update:
            # Get graph_id from the simulation state or from the project
            graph_id = state.graph_id
            if not graph_id:
                # Try to retrieve from the project
                project = ProjectManager.get_project(state.project_id)
                if project:
...
def get_run_status(simulation_id: str):
    """
    Retrieve the real-time run status of a simulation (used for frontend polling).
...
def get_run_status_detail(simulation_id: str):
    """
    Retrieve the detailed run status of a simulation (including all actions).

    Used for real-time frontend display.
    ...
    try:
        ...
        # Retrieve the full action list
        all_actions = SimulationRunner.get_all_actions(
            simulation_id=simulation_id,
            platform=platform_filter
        )

        # Retrieve actions per platform
        ...
        # Retrieve actions for the current round (recent_actions shows only the latest round)
        ...
        # Retrieve base status information
...
def get_simulation_actions(simulation_id: str):
    """
    Retrieve the action history of agents in a simulation.
...
def get_simulation_timeline(simulation_id: str):
    """
    Retrieve the simulation timeline (summarized by round).

    Used by the frontend to display the progress bar and timeline view.

    Query parameters:
        start_round: Starting round (default 0)
        end_round: Ending round (default: all rounds)

    Returns a summary for each round.
    """
    try:
...
def get_simulation_comments(simulation_id: str):
    """
    Retrieve comments from the simulation (Reddit only).
...
def interview_agent():
    ...
    (i.e., the simulation loop has completed and is waiting for commands).
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional: specify platform (twitter/reddit)
                                               // if omitted: dual-platform simulation interviews both platforms simultaneously
    ...
    try:
        ...
        # Optimize the prompt by prepending a prefix to prevent agents from calling tools
...
def interview_agents_batch():
            ...
            "interviews": [                    // required: list of interviews
                {
                    ...
                    "platform": "twitter"      // optional: platform for this agent
                },
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // omit platform to use the default
                }
            ],
            "platform": "reddit",              // optional: default platform (overridden per item)
                                               // if omitted: dual-platform simulation interviews each agent on both platforms
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate the per-item platform (if provided)
        ...
        # Optimize each interview's prompt by prepending a prefix to prevent agents from calling tools
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",                   // required: simulation ID
            "prompt": "What is your overall opinion?",     // required: interview question (same for all agents)
...
def get_interview_history():
    """
    Retrieve interview history records.
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // optional: platform type (reddit/twitter)
                                           // if omitted, returns history from both platforms
            "agent_id": 0,                 // optional: retrieve history for this agent only
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # if omitted, returns history from both platforms
        agent_id = data.get('agent_id')
...
def get_env_status():
    """
    Retrieve the simulation environment status.
    ...
    try:
        ...
        # Retrieve more detailed status information
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation, causing it to gracefully
    exit the command-waiting mode.

    Note: This differs from the /stop endpoint — /stop forcibly terminates the process,
    whereas this endpoint lets the simulation close its environment and exit gracefully.
