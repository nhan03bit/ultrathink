"""Zep Graph paginated read utilities.

The Zep node/edge list API uses UUID cursor-based pagination.
This module wraps the auto-pagination logic (with per-page retry),
returning a complete list transparently to callers.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page with exponential backoff retry on failure. Only retries transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, returning up to max_items (default 2000). Each page request includes retry logic."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all edges in the graph with pagination, returning a complete list. Each page request includes retry logic."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
