...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation completion (content only, does not indicate the full section is done)."""
        self.log(
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log entry to determine whether a section is truly
        complete and to obtain its full content.
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        """Set up a file handler to write logs to a file simultaneously."""
        import logging
...
This tool is used to obtain a complete overview of the simulation results, especially suitable
for understanding how events evolve. It will:
...
Calls the OASIS simulation environment's interview API to conduct real interviews with running
simulation agents! This is not an LLM simulation — it calls the real interview interface to
obtain raw answers from simulation agents. By default, interviews are conducted simultaneously
on both Twitter and Reddit for more comprehensive perspectives.
...
[Use Cases]
- Need to understand event views from different role perspectives (How do students see it? The media? Officials?)
...
simulated world — you can observe the behavior, statements, and interactions of every agent
in the simulation.

[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables.
The evolution of the simulated world is a prediction of what may happen in the future. What you
are observing is not "experimental data" but a "rehearsal of the future."
...
- ❌ Not a vague general overview of public opinion
...
1. Under the conditions we have set, what state does the future present?
...
(simulation requirements) into the simulated world, and the behavior and interactions of
agents in the simulation are predictions of future human behavior.
...
2. [Must Quote Agents' Raw Speech and Actions]
...
3. [Language Consistency - Quoted Content Must Be Translated to the Report Language]
   - Tool-returned content may contain expressions in a different language than the report
   - The entire report must be written in the language specified by the user
   - When quoting tool-returned content in another language, you must translate it to the report language before writing it in
...
- ❌ Prohibited: any Markdown headings inside a section (#, ##, ###, ####, etc.)
- ❌ Prohibited: adding the section's main heading at the beginning of content
- ✅ Section headings are added automatically by the system; you only need to write pure body text
- ✅ Use **bold**, paragraph breaks, blockquotes, and lists to organize content — but no headings

[Correct Example]
```
This section analyzes the public opinion spread of the event. Through in-depth analysis of the simulation data, we found...

**Initial Ignition Phase**

As the front line of public opinion, Weibo served as the core channel for first-breaking information:
...
- panorama_search: Wide-angle panoramic search, understand the full picture of the event, timeline, and evolution
...
In each reply you can only do one of the following two things (not both simultaneously):
...
Once you have gathered sufficient information through tools, output the section content starting with "Final Answer:"

⚠️ Strictly Prohibited:
- Prohibited: including both a tool call and Final Answer in the same reply
- Prohibited: fabricating tool return results (Observation) yourself — all tool results are injected by the system
...
3. Use Markdown format (but headings are prohibited):
   ...
   - ❌ Prohibited: any heading syntax (#, ##, ###, ####, etc.)
4. [Blockquote Format — Must Stand Alone as a Paragraph]
   Blockquotes must be independent paragraphs with a blank line before and after; they cannot be mixed into a paragraph:
...
3. After gathering sufficient information, output the Final Answer (pure body text, no headings)"""
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You have only called tools {tool_calls_count} time(s), but at least {min_tool_calls} are required. "
    "Please call more tools to retrieve simulation data, then output Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "You have only called tools {tool_calls_count} time(s) so far; at least {min_tool_calls} are required. "
...
2. Answer directly; avoid lengthy reasoning discussions
...
[Answer Style]
- Concise and direct; avoid lengthy responses
...
class ReportAgent:
    ...
    def __init__(
        ...
        # Structured logger (initialized in generate_report)
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Deep interview - call the real OASIS interview API to get simulation agent answers (dual platform)
            ...
            # ========== Legacy tools for backward compatibility (internally redirected to new tools) ==========
    ...
    # Set of valid tool names, used for validation when falling back to bare JSON parsing
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback — LLM outputs bare JSON directly (not wrapped in <tool_call> tags)
        # Only attempted when Format 1 did not match, to avoid false matches with JSON in body text
        ...
        # The response may contain thinking text + bare JSON; attempt to extract the last JSON object
    ...
    def plan_outline(
        ...
        Uses the LLM to analyze simulation requirements and plan the report structure.
