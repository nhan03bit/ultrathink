...
Dynamically updates agent activities from the simulation into the Zep knowledge graph.
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert the activity into a natural-language text description to send to Zep.
        ...
        # Return in "agent name: activity description" format without a simulation prefix
    ...
    def _describe_like_post(self) -> str:
        """Like a post — includes the original post content and author information."""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post — includes the original post content and author information."""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Repost a post — includes the original post content and author information."""
    ...
    def _describe_quote_post(self) -> str:
        """Quote a post — includes the original post content, author information, and the quote comment."""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_create_comment(self) -> str:
        """Post a comment — includes the comment content and information about the post being commented on."""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitors the simulation's actions log file and pushes new agent activities
    into the Zep knowledge graph in real time. Activities are grouped by platform
    and sent to Zep in batches once BATCH_SIZE activities have accumulated.

    All meaningful actions are sent to Zep. The action_args include full context:
    ...
    # Batch size — number of activities per platform to accumulate before sending
    BATCH_SIZE = 5

    # Platform display names (for console output)
    PLATFORM_DISPLAY_NAMES = {
        'twitter': 'World 1',
        'reddit': 'World 2',
    }

    # Send interval in seconds to avoid hitting rate limits
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._total_sent = 0        # successful batches sent to Zep
        self._total_items_sent = 0  # total individual activities sent to Zep
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will include full context (e.g., original post text, user names).
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Pause between sends to avoid hitting rate limits
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        # Merge multiple activities into a single newline-delimited text
    ...
    def _flush_remaining(self):
        """Send all remaining activities from the queue and buffers."""
        # First drain the queue into the platform buffers
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batches_sent": self._total_sent,            # successful batches sent
            "items_sent": self._total_items_sent,        # total individual activities sent
...
class ZepGraphMemoryManager:
    ...
    # Guard flag to prevent stop_all from being called multiple times
