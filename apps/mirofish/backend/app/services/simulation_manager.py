...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from the Zep knowledge graph
        2. Generate an OASIS Agent Profile for each entity (optional LLM enhancement, parallel supported)
        ...
        Args:
            ...
            document_text: Original document content (used by LLM to understand the background)
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No entities matching the criteria were found. Please verify that the knowledge graph is built correctly."
                self._save_simulation_state(state)
                return state
            ...
            # Reddit profiles are already saved in real time during generation; saving again here ensures completeness
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter requires CSV format — this is mandated by OASIS
                generator.save_profiles(
                    profiles=profiles,
            ...
            # ========== Stage 3: LLM-powered simulation config generation ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0,
            ...
            # When starting a simulation, simulation_runner executes scripts from the scripts/ directory.
