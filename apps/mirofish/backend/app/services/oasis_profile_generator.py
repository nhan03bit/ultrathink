...
2. Optimized prompts for generating highly detailed personas
...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep knowledge graph into Agent Profiles required for OASIS simulation
    ...
    # Individual-type entities (require generating specific personas)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/institution-type entities (require generating representative group personas)
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        if use_llm:
            ...
            # Use rule-based generation for basic persona
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search interface; edges and nodes must be
        searched separately and results merged. Parallel requests are used to improve efficiency.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        2. Detailed information from associated nodes
        ...
        # 3. Add detailed information from associated nodes
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine whether this is a group/institution-type entity"""
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Do not set max_tokens to allow LLM full freedom
                )
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First attempt to fix truncation
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            # 3. Handle newline issues within strings
        ...
        # 6. Attempt to extract partial information from content
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user profiles. Generate detailed, authentic personas for public opinion simulation, staying as faithful as possible to existing real-world information. You must return valid JSON format; no string value may contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity below, staying as faithful as possible to existing real-world information.
...
2. persona: Detailed persona description (2000 words of plain text), including:
   ...
   - Stance and opinions (attitude toward topics, content likely to provoke or move them)
...
6. country: Country name (e.g. "China", "United States")
...
- persona must be a single cohesive block of text
- {get_language_instruction()} (the gender field must use the English values male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institution/group entity, staying as faithful as possible to existing real-world information.
...
2. persona: Detailed account profile description (2000 words of plain text), including:
   - Basic institution information (formal name, nature of institution, founding background, main functions)
   ...
   - Special notes (demographic profile of the represented group, operational habits)
   - Institutional memory (an important part of the persona — describe this institution's connection to events, and its existing actions and reactions within those events)
3. age: Fixed value of 30 (virtual age for institutional accounts)
...
8. interested_topics: Array of fields of interest

Important:
- All field values must be strings or numbers; null values are not allowed
- persona must be a single cohesive block of text; do not use newline characters
- {get_language_instruction()} (the gender field must use the English value "other")
- age must be the integer 30; gender must be the string "other"
- The institutional account's communication should match its identity and positioning"""
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Build the full output (no truncation)
        ...
        # Output to console only (avoid duplication — logger no longer prints the full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save profiles to a file (using the correct format for the given platform).
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles to CSV format (compliant with the OASIS official specification).
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Replace newlines with spaces (newlines not allowed in CSV values)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Mapping for Chinese and English values
        gender_map = {
            "男": "male",
            "女": "female",
            "机构": "other",
            "其他": "other",
            # English values already accepted
            "male": "male",
            "female": "female",
            "other": "other",
        }
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        The user_id field is required — it is the key used by OASIS agent_graph.get_agent() for matching.
    ...
    # Retain the old method name as an alias for backward compatibility
