...
class SimulationRunState:
    ...
    # Per-platform independent round counts and simulated time (for dual-platform parallel display)
    ...
    # Recent actions (for real-time display on the frontend)
    ...
    # Process ID (used to stop the simulation)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform independent round counts and simulated time
...
class SimulationRunner:
    ...
    # Directory for storing run state
    ...
    def start_simulation(
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to dynamically push Agent activity to the Zep graph
        ...
        # If max_rounds is specified, truncate the total rounds accordingly
        ...
        # Determine which script to run (scripts are located in the backend/scripts/ directory)
        if platform == "twitter":
        ...
        try:
            ...
            cmd = [
                ...
                script_path,
                "--config", config_path,  # Full path to the configuration file
            ]

            # If max_rounds is specified, append it to the command-line arguments
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])

            # Create the main log file to avoid blocking the process when the stdout/stderr pipe buffer fills up
            ...
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+: makes all open() calls default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8

            # Set the working directory to the simulation directory (database files, etc. are created there).
            # Use start_new_session=True to create a new process group so all child processes
            # can be terminated via os.killpg.
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # stderr is written to the same file
                text=True,
                encoding='utf-8',  # Explicitly specify encoding
                bufsize=1,
                env=env,  # Pass the environment with UTF-8 settings
                start_new_session=True,  # Create a new process group so all related processes can be terminated when the server shuts down
            )

            # Save file handles for later cleanup
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # A separate stderr handle is no longer needed
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # Do a final log read after the process ends
            if os.path.exists(twitter_actions_log):
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detect simulation_end events and mark the platform as completed
                                ...
                                # Update round information (from round_end events)
                                    ...
                                    # Overall round count is the maximum across both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall simulated time is the maximum across both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        A platform is considered enabled if its corresponding actions.jsonl file exists.
        ...
        # Determine which platforms are enabled (by checking whether the files exist)
        twitter_enabled = os.path.exists(twitter_log)
        reddit_enabled = os.path.exists(reddit_log)

        # If a platform is enabled but not yet completed, return False
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Terminate a process and its children in a cross-platform manner.
        ...
        """
        if IS_WINDOWS:
            # Windows: use the taskkill command to terminate the process tree
            ...
            # Because start_new_session=True was used, the process group ID equals the main process PID.
            ...
            try:
                ...
                # If still running after the timeout, force-send SIGKILL
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when the action record has no platform field)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Resolve the platform: prefer the value in the record, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        """
        Retrieve the complete action history for all platforms (no pagination limit).
        ...
        # Read the Twitter action file (platform is automatically set to "twitter" based on file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # Automatically fill in the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))

        # Read the Reddit action file (platform is automatically set to "reddit" based on file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # Automatically fill in the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))

        # If per-platform files are not found, fall back to the old single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # The old format files should already contain the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_actions(
        ...
        """
        Retrieve action history with pagination.
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up the simulation's run logs (used to force a fresh simulation start).
        ...
        Note: configuration files (simulation_config.json) and profile files are not deleted.
        ...
        # Clean up action logs inside platform directories
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean up (avoids printing unnecessary logs when no processes exist)
        ...
        # Stop all graph memory updaters first (stop_all prints its own log messages)
        try:
        ...
        # Copy the dict to avoid modifying it during iteration
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        ...
                        # Process may no longer exist; attempt direct termination
                        try:
                            process.terminate()
                    ...
                    # Also update state.json and set status to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        Call this at Flask application startup to ensure all simulation processes
        are cleaned up when the server shuts down.
        """
        ...
        # In Flask debug mode, only register cleanup in the reloader subprocess (the one that actually runs the app).
        # WERKZEUG_RUN_MAIN=true indicates the reloader subprocess.
        # In non-debug mode this variable is absent, but cleanup still needs to be registered.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode, register only in the reloader subprocess; always register in non-debug mode.
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent subprocesses from retrying
            return

        # Save the existing signal handlers
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP only exists on Unix systems (macOS/Linux); Windows does not have it
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler: clean up simulation processes first, then invoke the original handler."""
            # Only log if there are processes to clean up
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"Received signal {signum}, starting cleanup...")
            cls.cleanup_all_simulations()

            # Invoke the original signal handler so Flask can exit normally
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If the original handler is not callable (e.g. SIG_DFL), use the default behaviour
        ...
        try:
            # SIGTERM: default signal sent by the kill command
    ...
    def get_running_simulations(cls) -> List[str]:
        """
        Return a list of all currently running simulation IDs.
        """
        running = []
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if the environment is alive; False if it has been shut down.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Retrieve detailed status information about the simulation environment.
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: interview on both platforms simultaneously (dual-platform mode)
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or has been shut down; cannot execute interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional; overridden per interview item)
                - "twitter": interview on Twitter by default
                - "reddit": interview on Reddit by default
                - None: interview each agent on both platforms simultaneously (dual-platform mode)
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Asks all agents in the simulation the same question.
        ...
        # Load all agent information from the config file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        try:
            ...
            # Timeout may indicate the environment is already shutting down
            return {
                "success": True,
    ...
    def _get_interview_history_from_db(
        ...
        """Retrieve interview history from a single database."""
        import sqlite3
    ...
    def get_interview_history(
        ...
        """
        Retrieve interview history records (read from the database).

        Args:
            simulation_id: Simulation ID
            platform: Platform type (reddit/twitter/None)
                - "reddit": retrieve Reddit history only
                - "twitter": retrieve Twitter history only
                - None: retrieve history from both platforms
            agent_id: Agent ID (optional; retrieve history for this agent only)
        ...
        if platform in ("reddit", "twitter"):
            ...
            # If no platform is specified, query both platforms
        ...
        # If multiple platforms were queried, cap the total result count
