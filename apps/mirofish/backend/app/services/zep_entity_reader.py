...
Reads nodes from the Zep knowledge graph and filters those matching predefined entity types
"""
...
class ZepEntityReader:
    ...
    Main features:
    1. Read all nodes from the Zep knowledge graph
    ...
    def _call_with_retry(
        ...
        Args:
            func: Function to execute (a no-argument lambda or callable)
            operation_name: Operation name used in log messages
            max_retries: Maximum number of retries (default 3, meaning at most 3 attempts)
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges in the knowledge graph (paginated)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all edges related to a specific node (with retry logic)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's Labels contain only "Entity", it does not match any predefined type — skip it
        ...
        # Get all edges (for later relationship lookups)
        ...
        for node in all_nodes:
            ...
            # Filtering logic: Labels must include at least one label other than "Entity" and "Node"
            ...
            # If predefined types are specified, check whether they match
            ...
            # Retrieve related edges and nodes
            if enrich_with_edges:
                ...
                # Retrieve basic info for related nodes
                related_nodes = []
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity along with its full context (edges and associated nodes, with retry logic)
        ...
        try:
            ...
            # Get all nodes for relationship lookups
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
            ...
            # Retrieve related node information
            related_nodes = []
            for related_uuid in related_node_uuids:
                if related_uuid in node_map:
                    related_node = node_map[related_uuid]
