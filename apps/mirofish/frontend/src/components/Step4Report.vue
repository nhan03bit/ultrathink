            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation done, but the full section may not be complete yet) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When this is the final answer, show a special indicator -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After the DOM updates, adjust scroll position to keep the button in the same place
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // users icon - represents conversation/interview
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extract analysis query (backend API key: "分析问题:")
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extract prediction scenario (backend API key: "预测场景:")
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Extract stats - matching "相关预测事实: X条" format (backend API keys)
    ...
    // Extract sub-questions - full extraction, no count limit (backend API section: "### 分析的子问题")
    ...
    // Extract key facts - full extraction, no count limit (backend API section: "### 【关键事实】")
    ...
    // Extract core entities - full extraction, including summary and related fact count (backend API section: "### 【核心实体】")
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/相关事实:\s*(\d+)/) // backend API key: "相关事实:"
        return {
    ...
    // Extract relation chains - full extraction, no count limit (backend API section: "### 【关系链】")
...
const parsePanorama = (text) => {
  ...
  try {
    // Extract query (backend API key: "查询:")
    const queryMatch = text.match(/查询:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extract stats (backend API keys)
    ...
    // Extract current active facts - full extraction, no count limit (backend API section: "### 【当前有效事实】")
    ...
    // Extract historical/expired facts - full extraction, no count limit (backend API section: "### 【历史/过期事实】")
    ...
    // Extract involved entities - full extraction, no count limit (backend API section: "### 【涉及实体】")
...
const parseInterview = (text) => {
  ...
  try {
    // Extract interview topic (backend API key: "**采访主题:**")
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()

    // Extract interviewee count (e.g. "5 / 9 位模拟Agent") (backend API key: "**采访人数:**")
    ...
    // Extract selection reason for interviewees (backend API section: "### 采访对象选择理由")
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **校友_345（index=1）**：作为武大校友... (backend API format)
        ...
        // Format 2: - 选择name（index X）：reason (backend API format)
        // e.g.: - 选择家长_601（index 0）：作为家长群体代表...
        if (!headerMatch) {
        ...
        // Format 3: - **name（index X）**：reason (backend API format)
        // e.g.: - **家长_601（index 0）**：作为家长群体代表...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation line for the reason (exclude trailing summary paragraphs)
    ...
    // Extract each interview record (backend API section header: "#### 采访 #N:")
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g. "学生", "教育从业者" — backend API values, kept as-is)
      ...
      // Extract bio (backend API key: "_简介:..._")
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question starts with "1.", handle it specially
      ...
      // Extract answers - split into Twitter and Reddit (backend API keys: "【Twitter平台回答】", "【Reddit平台回答】")
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatibility with old format: only one platform marker present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer — copy as default display if it's not placeholder text
          ...
          // No platform markers (very old format) — use entire answer
          interview.twitterAnswer = answerText
        }
      }

      // Extract key quotes (compatible with multiple quote formats) (backend API key: "**关键引言:**")
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Prefer matching > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: match > "text" or > \u201Ctext\u201D (curly/Chinese quotes)
        if (!quoteMatches) {
    ...
    // Extract interview summary (backend API section: "### 采访摘要与核心观点")
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Extract search query (backend API key: "搜索查询:")
    const queryMatch = text.match(/搜索查询:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extract result count (backend API format: "找到 N 条")
    const countMatch = text.match(/找到\s*(\d+)\s*条/)
    if (countMatch) result.count = parseInt(countMatch[1])

    // Extract related facts - full extraction, no count limit (backend API section: "### 相关事实:")
    ...
    // Try to extract edge information if present (backend API section: "### 相关边:")
    ...
    // Try to extract node information if present (backend API section: "### 相关节点:")
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether the text is a platform placeholder (backend API values, kept as-is for comparison)
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Prefer "问题X：" format (backend API key)
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // If no numbering found or only one match, return the whole answer
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^问题\d+[：:]\s*/, '') // backend API key: "问题N:"
          .replace(/^\d+\.\s+/, '')
          .trim()
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return the full answer for the first question, empty for the rest
      return qIdx === 0 ? answer : ''
    }

    // Check whether a question has answers on both platforms (filter placeholder text)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle button (only shown when both platforms have real answers)
...
// Current active step (used for the top display)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading level-2 heading (## xxx) since section titles are already shown in the outer wrapper
  ...
  // Remove all whitespace between list items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Remove whitespace after list opening tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Remove whitespace before list closing tags
  ...
  // Handle line breaks - blank lines become paragraph separators, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Remove <br> tags before and after block-level elements
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Remove <p><br> immediately before block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Remove consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove <br> immediately before a paragraph opening tag after block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol>s are separated by paragraph content, keep incrementing
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('错误')) return 'error' // '错误' = backend Chinese for ERROR
  if (log.includes('WARNING') || log.includes('警告')) return 'warning' // '警告' = backend Chinese for WARNING
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled uniformly in nextTick after the loop ends
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is complete, scroll to top; otherwise scroll to bottom to follow latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "最终答案:" (backend API key, kept as-is)
  ...
  // If content is long and contains markdown, try removing the thinking preamble before returning
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the "Thought:" thinking preamble
