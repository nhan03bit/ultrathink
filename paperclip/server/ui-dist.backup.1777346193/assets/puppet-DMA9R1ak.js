var c = {},
  s = /({)?([a-z][a-z0-9_]*)?((::[a-z][a-z0-9_]*)*::)?[a-zA-Z0-9_]+(})?/;
function a(e, n) {
...
function l(e, n) {
  ...
  return n.inInclude
    ? (e.match(/(\s+)?\S+(\s+)?/), (n.inInclude = !1), "def")
    : e.match(/(\s+)?\w+\(/)
      ? (e.backUp(1), "def")
      : o
        ? (e.match(/(\s+)?\w+/), "tag")
        : i && c.hasOwnProperty(i)
          ? (e.backUp(1),
            e.match(/[\w]+/),
            e.match(/\s+\S+\s+{/, !1) && (n.inDefinition = !0),
            i == "include" && (n.inInclude = !0),
            c[i])
          : /(^|\s+)[A-Z][\w:_]+/.test(i)
            ? (e.backUp(1), e.match(/(^|\s+)[A-Z][\w:_]+/), "def")
            : t
              ? (e.match(/(\s+)?[\w:_]+/), "def")
              : f
                ? (e.match(/(\s+)?[@]{1,2}/), "atom")
                : r == "#"
                  ...
                  : r == "'" || r == '"'
                    ...
                    : r == "{" || r == "}"
                      ? "bracket"
                      : r == "/"
                        ? (e.match(/^[^\/]*\//), "string.special")
                        : r.match(/[0-9]/)
                          ? (e.eatWhile(/[0-9]+/), "number")
                          : r == "="
                            ? (e.peek() == ">" && e.next(), "operator")
                            : (e.eatWhile(/[\w-]/), null);
}
const p = {
  name: "puppet",
