function a(e, n) {
  n = n || {};
  for (
    var r = n.prefix !== void 0 ? n.prefix : "^", t = n.suffix !== void 0 ? n.suffix : "\\b", i = 0;
    i < e.length;
    i++
  )
...
var f = "(?=[^A-Za-z\\d\\-_]|$)",
  ...
  C = /[+\-*\/%]=|\+\+|--|\.\.|[+\-*&^%:=!|\/]|<(?!#)|(?!#)>/,
  k = a([b, C], { suffix: "" }),
  h = /^((0x[\da-f]+)|((\d+\.\d+|\d\.|\.\d+|\d+)(e[\+\-]?\d+)?))[ld]?([kmgtp]b)?/i,
  x = /^[A-Za-z\_][A-Za-z\-\_\d]*\b/,
  E = /[A-Z]:|%|\?/i,
  w = a(
    [
  ...
  M = a([E, w, y], { suffix: f }),
  p = { keyword: P, number: h, operator: k, builtin: M, punctuation: d, variable: x };
function o(e, n) {
  var r = n.returnStack[n.returnStack.length - 1];
  if (r && r.shouldReturnFrom(n)) return ((n.tokenize = r.tokenize), n.returnStack.pop(), n.tokenize(e, n));
  if (e.eatSpace()) return null;
  if (e.eat("(")) return ((n.bracketNesting += 1), "punctuation");
  if (e.eat(")")) return ((n.bracketNesting -= 1), "punctuation");
  for (var t in p) if (e.match(p[t])) return t;
  var i = e.next();
  if (i === "'") return R(e, n);
  if (i === "$") return s(e, n);
  if (i === '"') return v(e, n);
  if (i === "<" && e.eat("#")) return ((n.tokenize = S), S(e, n));
  if (i === "#") return (e.skipToEnd(), "comment");
  if (i === "@") {
    var l = e.eat(/["']/);
    if (l && e.eol()) return ((n.tokenize = u), (n.startQuote = l[0]), u(e, n));
    if (e.eol()) return "error";
    if (e.peek().match(/[({]/)) return "punctuation";
    if (e.peek().match(c)) return s(e, n);
  }
  return "error";
}
function R(e, n) {
  for (var r; (r = e.peek()) != null; ) if ((e.next(), r === "'" && !e.eat("'"))) return ((n.tokenize = o), "string");
  return "error";
}
function v(e, n) {
  for (var r; (r = e.peek()) != null; ) {
    if (r === "$") return ((n.tokenize = I), "string");
    if ((e.next(), r === "`")) {
      e.next();
      continue;
    }
    if (r === '"' && !e.eat('"')) return ((n.tokenize = o), "string");
  }
  return "error";
}
function I(e, n) {
...
function s(e, n) {
  ...
  return e.eat("{")
    ? ((n.tokenize = m), m(e, n))
    : r != null && r.match(c)
      ? (e.eatWhile(c), (n.tokenize = o), "variable")
      : ((n.tokenize = o), "error");
}
function m(e, n) {
...
function u(e, n) {
  var r = n.startQuote;
  if (e.sol() && e.match(new RegExp(r + "@"))) n.tokenize = o;
  else if (r === '"')
    for (; !e.eol(); ) {
      var t = e.peek();
      if (t === "$") return ((n.tokenize = T), "string");
      (e.next(), t === "`" && e.next());
    }
  else e.skipToEnd();
  return "string";
...
const D = {
  name: "powershell",
  startState: function () {
    return { returnStack: [], bracketNesting: 0, tokenize: o };
  },
  token: function (e, n) {
