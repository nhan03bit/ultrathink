function o(t) {
  ...
  t
    ? ((n =
      ...
      (l = /^(((call|j[pr]|rst|ret[in]?)(\.([sl]?i)?[sl])?)|(rs|st)mix)\b/i))
    : ((n =
      ...
      (l = /^(call|j[pr]|ret[in]?|b_?(call|jump))\b/i));
  var u = /^(af?|bc?|c|de?|e|hl?|l|i[xy]?|r|sp)\b/i,
    d = /^(n?[zc]|p[oe]?|m)\b/i,
    f = /^([hl][xy]|i[xy][hl]|slia|sll)\b/i,
    c = /^([\da-f]+h|[0-7]+o|[01]+b|\d+d?)\b/i;
  return {
    name: "z80",
    startState: function () {
      return { context: 0 };
