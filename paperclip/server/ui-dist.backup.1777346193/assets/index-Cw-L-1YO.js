import { L as s, E as R, a as Y, C as x } from "./index-Bwdi7UEW.js";
import {
  s as w,
  t as O,
  a as d,
...
const _ = 177,
  ...
  aO = new R(
    (Q, e) => {
      for (
        let n = 0, a = Q.next;
        ((e.context && (a < 0 || a == M || a == H || (a == $ && Q.peek(n + 1) == $))) || a == QO || a == eO) &&
          Q.acceptToken(_),
          !(a != K && a != OO);
      )
        a = Q.peek(++n);
...
let tO = new Set([N, E, z, v, D, L, A, I, J, C, B, F]);
const iO = new x({ start: !1, shift: (Q, e) => (e == q ? Q : tO.has(e)) }),
  XO = w({
    ...
    DefName: O.definition(O.variableName),
    TypeName: O.typeName,
    LabelName: O.labelName,
    FieldName: O.propertyName,
    "FunctionDecl/DefName": O.function(O.definition(O.variableName)),
    "TypeSpec/DefName": O.definition(O.typeName),
    "CallExpr/VariableName": O.function(O.variableName),
    ...
    "DerefOp .": O.derefOperator,
    "UpdateOp IncDecOp": O.updateOperator,
    CompareOp: O.compareOperator,
    "= :=": O.definitionOperator,
    "<-": O.operator,
    '~ "*"': O.modifier,
    "; ,": O.separator,
    "... :": O.punctuation,
  ...
  oO = [
    X("func ${name}(${params}) ${type} {\n	${}\n}", { label: "func", detail: "declaration", type: "keyword" }),
    X("func (${receiver}) ${name}(${params}) ${type} {\n	${}\n}", {
      label: "func",
      detail: "method declaration",
      type: "keyword",
    }),
    X("var ${name} = ${value}", { label: "var", detail: "declaration", type: "keyword" }),
    X("type ${name} ${type}", { label: "type", detail: "declaration", type: "keyword" }),
    X("const ${name} = ${value}", { label: "const", detail: "declaration", type: "keyword" }),
    X("type ${name} = ${type}", { label: "type", detail: "alias declaration", type: "keyword" }),
    X("for ${init}; ${test}; ${update} {\n	${}\n}", { label: "for", detail: "loop", type: "keyword" }),
    X("for ${i} := range ${value} {\n	${}\n}", { label: "for", detail: "range", type: "keyword" }),
    X(
      `select {
	\${}
}`,
      { label: "select", detail: "statement", type: "keyword" }
    ),
    X("case ${}:\n${}", { label: "case", type: "keyword" }),
    X("switch ${} {\n	${}\n}", { label: "switch", detail: "statement", type: "keyword" }),
    X("switch ${}.(${type}) {\n	${}\n}", { label: "switch", detail: "type statement", type: "keyword" }),
    ...
    X('import ${name} "${module}"\n${}', { label: "import", detail: "declaration", type: "keyword" }),
  ],
  S = new b(),
  T = new Set([
    "SourceFile",
...
function o(Q, e) {
  return (n, a) => {
    O: for (let t = n.node.firstChild, c = 0, i = null; ; ) {
      for (; !t; ) {
        if (!c) break O;
        (c--, (t = i.nextSibling), (i = i.parent));
      }
      (e && t.name == e) || t.name == "SpecList"
        ? (c++, (i = t), (t = t.firstChild))
        : (t.name == "DefName" && a(t, Q), (t = t.nextSibling));
    }
    return !0;
  };
}
const cO = {
  FunctionDecl: o("function"),
  VarDecl: o("var", "VarSpec"),
  ConstDecl: o("constant", "ConstSpec"),
  TypeDecl: o("type", "TypeSpec"),
  ImportDecl: o("constant", "ImportSpec"),
  Parameter: o("var"),
  __proto__: null,
};
function m(Q, e) {
  ...
  return (
    e.cursor(Z.IncludeAnonymous).iterate((i) => {
      if (t) t = !1;
      else if (i.name) {
        let P = cO[i.name];
        if ((P && P(i, c)) || T.has(i.name)) return !1;
      } else if (i.to - i.from > 8192) {
        for (let P of m(Q, i.node)) a.push(P);
        return !1;
      }
    }),
    S.set(e, a),
...
const p = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,
  W = ["String", "LineComment", "BlockComment", "DefName", "LabelName", "FieldName", ".", "?."],
  rO = (Q) => {
    let e = G(Q.state).resolveInner(Q.pos, -1);
    if (W.indexOf(e.name) > -1) return null;
    let n = e.name == "VariableName" || (e.to - e.from < 20 && p.test(Q.state.sliceDoc(e.from, e.to)));
    if (!n && !Q.explicit) return null;
    let a = [];
    for (let t = e; t; t = t.parent) T.has(t.name) && (a = a.concat(m(Q.state.doc, t)));
    return { options: a, from: n ? e.from : Q.pos, validFor: p };
  },
  r = f.define({
    name: "go",
    parser: PO.configure({
...
let lO = (Q) => ({ label: Q, type: "keyword" });
const $O = "interface struct chan map package go return break continue goto fallthrough else defer range true false nil"
  .split(" ")
  .map(lO);
function TO() {
  let Q = oO.concat($O);
  return new d(r, [r.data.of({ autocomplete: k(W, h(Q)) }), r.data.of({ autocomplete: rO })]);
}
export { TO as go, r as goLanguage, rO as localCompletionSource, oO as snippets };
