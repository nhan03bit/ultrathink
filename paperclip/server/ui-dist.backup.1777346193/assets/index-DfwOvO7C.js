...
import { html as j } from "./index-CRYcSL6M.js";
import { L as V, E as c } from "./index-Bwdi7UEW.js";
      ...
      if (n == 123) {
        ...
        for (;;) {
          ...
          if (e == 32 || e == 10) ++$;
            ...
            let t = e == 101 && O.peek($ + 1) == 110 && O.peek($ + 2) == 100;
            O.acceptToken(t ? w : k, i);
            return;
          }
        }
      }
    }
    if ((O.advance(), r == 10)) break;
  }
  O.pos > a && O.acceptToken(v);
});
function u(O, a, r) {
  return new c((n) => {
    let $ = n.pos;
    for (;;) {
      ...
      if ((n.advance(), i == 10)) break;
    }
    n.pos > $ && n.acceptToken(a);
  });
}
const C = u("endcomment", z, E),
  Y = u("endraw", _, R),
  N = new c((O) => {
    if (O.next == 35) {
...
function l(O, a) {
  return O.split(" ").map((r) => ({ label: r, type: a }));
}
const P = l(
  ...
  A = l("first index index0 last length rindex", "property"),
  B = l("col col0 col_first col_last first index index0 last length rindex rindex0 row", "property");
function M(O) {
  var a;
  let { state: r, pos: n } = O,
    $ = y(r).resolveInner(n, -1).enterUnfinishedNodesBefore(n),
    i = ((a = $.childBefore(n)) === null || a === void 0 ? void 0 : a.name) || $.name;
  ...
  let e = O.matchBefore(/[\w\u00c0-\uffff]+$/);
  return e
    ? { type: "expression", from: e.from }
    : O.explicit &&
        $.name != "CommentText" &&
        $.name != "StringLiteral" &&
        $.name != "NumberLiteral" &&
...
function K(O, a, r, n) {
  ...
  for (;;) {
    ...
    if (!i) return [];
    if (i.name == "VariableName" || i.name == "forloop" || i.name == "tablerowloop") {
      let e = O.sliceDoc(i.from, i.to);
      if (e == "forloop") return $.length ? [] : A;
      if (e == "tablerowloop") return $.length ? [] : B;
      $.unshift(e);
      break;
    } else if (i.name == "MemberExpression") {
      let e = i.getChild("PropertyName");
      (e && $.unshift(O.sliceDoc(e.from, e.to)), (a = i));
    } else if (i.name == "SubscriptExpression") {
      let e = i.getChildren("Expression")[1];
      ($.unshift((e == null ? void 0 : e.name) == "StringLiteral" ? O.sliceDoc(e.from + 1, e.to - 1) : "[]"), (a = i));
    } else return [];
  }
  return n ? n($, O, r) : [];
}
function J(O = {}) {
  let a = O.filters ? O.filters.concat(P) : P,
    r = O.tags ? O.tags.concat(m) : m,
    n = O.variables ? O.variables.concat(T) : T,
    { properties: $ } = O;
  return (i) => {
    var e;
    let t = M(i);
    if (!t) return null;
    let o = (e = t.from) !== null && e !== void 0 ? e : t.node ? t.node.from : i.pos,
      s;
    return (
      t.type == "filter"
        ? (s = a)
        : t.type == "tag"
          ...
          : t.type == "expression"
            ? (s = n)
            : (s = K(i.state, t.target, i, $)),
      s.length ? { options: s, from: o, validFor: /^[\w\u00c0-\uffff]*$/ } : null
    );
  };
}
const OO = f.inputHandler.of((O, a, r, n) =>
  n != "%" || a != r || O.state.doc.sliceString(a - 1, r + 1) != "{}"
    ? !1
    : (O.dispatch(
...
function d(O) {
  return (a) => {
    let r = O.test(a.textAfter);
    return a.lineIndent(a.node.from) + (r ? 0 : a.unit);
  };
}
const $O = h.define({
    name: "liquid",
    parser: H.configure({
      props: [
        G({
          "cycle comment endcomment raw endraw echo increment decrement liquid in with as": Q.keyword,
          "empty forloop tablerowloop": Q.atom,
          "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue":
            Q.controlKeyword,
          "assign capture endcapture": Q.definitionKeyword,
          contains: Q.operatorKeyword,
          "render include": Q.moduleKeyword,
          VariableName: Q.variableName,
          TagName: Q.tagName,
          FilterName: Q.function(Q.variableName),
          ...
          StringLiteral: Q.string,
          BooleanLiteral: Q.bool,
          InlineComment: Q.lineComment,
          CommentText: Q.blockComment,
          "{% %} {{ }}": Q.brace,
          "[ ]": Q.bracket,
          "( )": Q.paren,
          ".": Q.derefOperator,
        ...
        b.add({
          "UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(
            ...
            let a = O.firstChild,
              r = O.lastChild;
            return !a || a.name != "Tag" ? null : { from: a.to, to: r.name == "EndTag" ? r.from : O.to };
          },
        }),
      ],
    }),
...
function g(O) {
  return $O.configure(
    {
      wrap: X((a) =>
        a.type.isTop ? { parser: O.parser, overlay: (r) => r.name == "Text" || r.name == "RawText" } : null
      ),
    },
    "liquid"
  );
}
const aO = g(q.language);
function tO(O = {}) {
  let a = O.base || q,
    r = a.language == q.language ? aO : g(a.language);
  return new x(r, [
    a.support,
    r.data.of({ autocomplete: J(O) }),
    a.language.data.of({ closeBrackets: { brackets: ["{"] } }),
    OO,
  ]);
}
export { OO as closePercentBrace, tO as liquid, J as liquidCompletionSource, aO as liquidLanguage };
