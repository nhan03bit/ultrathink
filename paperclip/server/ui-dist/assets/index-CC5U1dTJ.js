import { L as W, E as i, C as B } from "./index-CW2e1zPo.js";
import { s as U, t as n, a as b, p as E, L as C, i as u, h as v, f as M, j as N } from "./index-hX5IdXBW.js";
const c = 63,
  D = 64,
  j = 1,
  A = 2,
...
class s {
  constructor(e, a, r) {
    ((this.parent = e),
      (this.depth = a),
      (this.type = r),
      (this.hash = (e ? (e.hash + e.hash) << 8 : 0) + a + (a << 4) + r));
  }
}
...
const cO = new B({
  ...
  shift(O, e, a, r) {
    if (e == y) return new s(O, X(r, r.pos), T);
    if (e == z || e == Z) return new s(O, X(r, r.pos), g);
    if (e == c) return O.parent;
    if (e == tO || e == PO) return new s(O, 0, d);
    if (e == V && O.type == m) return O.parent;
    if (e == sO) {
...
const fO = new i(
    (O, e) => {
      if (O.next == -1 && e.canShift(D)) return O.acceptToken(D);
      let a = O.peek(-1);
      if ((P(a) || a < 0) && e.context.type != d) {
        ...
        let r = 0;
        for (; O.next == 32; ) (r++, O.advance());
        (r < e.context.depth || (r == e.context.depth && e.context.type == T && (O.next != 45 || !l(O.peek(1))))) &&
          O.next != -1 &&
          !P(O.next) &&
          O.next != 35 &&
          O.acceptToken(c, -r);
...
function Y(O, e) {
  return O.next == 37
    ? (O.advance(), q(O.next) && O.advance(), q(O.next) && O.advance(), !0)
    : dO(O.next) || (e && O.next == 44)
      ? (O.advance(), !0)
      : !1;
}
function G(O) {
...
function p(O, e) {
  ...
  for (O.advance(); ; ) {
    ...
    else if (t == 92 && a == 34) O.next >= 0 && O.advance();
    else if (P(t)) {
      if (e) return !1;
      r = !0;
    } else if (e && O.pos >= o + 1024) return !1;
  }
  return !r;
}
function RO(O) {
  for (let e = [], a = O.pos + 1024; ; )
    if (O.next == 91 || O.next == 123) (e.push(O.next), O.advance());
    else if (O.next == 39 || O.next == 34) {
      if (!p(O, !0)) return !1;
      ...
      if (O.next < 0 || O.pos > a || P(O.next)) return !1;
      O.advance();
    }
}
...
function w(O, e, a, r) {
  if (S(O.next) == "s" || ((O.next == 63 || O.next == 58 || O.next == 45) && k(O.peek(1), a))) O.advance();
  else return !1;
  let o = O.pos;
  for (;;) {
    let t = O.next,
    ...
    if (
      !(t >= 0 && (t == 58 ? k(O.peek(Q + 1), a) : t == 35 ? O.peek(Q - 1) != 32 : k(t, a))) ||
      (!a && R <= r) ||
      (R == 0 && !a && (f(O, 45, Q) || f(O, 46, Q)))
    )
      break;
    if (e && S(t) == "f") return !1;
...
const iO = new i((O, e) => {
    if (O.next == 33) (G(O), O.acceptToken(rO));
    else if (O.next == 38 || O.next == 42) {
      let a = O.next == 38 ? eO : aO;
      ($(O), O.acceptToken(a));
    } else
      O.next == 39 || O.next == 34
        ? (p(O, !1), O.acceptToken(OO))
        : w(O, !1, e.context.type == d, e.context.depth) && O.acceptToken(J);
  }),
  kO = new i((O, e) => {
    let a = e.context.type == m ? e.context.depth : -1,
      r = O.pos;
    O: for (;;) {
      let o = 0,
        t = O.next;
  ...
  mO = W.deserialize({
    ...
    repeatNodeCount: 2,
    tokenData:
      "$z~RXOYnYZ!^Z]n]^!^^}n}!O!i!O;'Sn;'S;=`!c<%lOn~qXOYnYZ!^Z]n]^!^^;'Sn;'S;=`!c<%l~n~On~~!^~!cOY~~!fP;=`<%ln~!lZOYnYZ!^Z]n]^!^^}n}!O#_!O;'Sn;'S;=`!c<%l~n~On~~!^~#bZOYnYZ!^Z]n]^!^^}n}!O$T!O;'Sn;'S;=`!c<%l~n~On~~!^~$WXOYnYZ$sZ]n]^$s^;'Sn;'S;=`!c<%l~n~On~~$s~$zOX~Y~",
    tokenizers: [0],
        ...
        u.add({
          Stream: (O) => {
            for (let e = O.node.resolve(O.pos, -1); e && e.to >= O.pos; e = e.parent) {
              ...
              if (e.name == "BlockSequence" || e.name == "BlockMapping") return O.column(e.firstChild.from, 1);
              if (e.name == "QuotedLiteral") return null;
              if (e.name == "Literal") {
                let a = O.column(e.from, 1);
                if (a == O.lineIndent(e.from, 1)) return a;
                if (e.to > O.pos) return null;
              }
            }
            return null;
          },
          FlowMapping: v({ closing: "}" }),
          FlowSequence: v({ closing: "]" }),
        }),
        M.add({
...
const xO = C.define({ name: "yaml-frontmatter", parser: mO.configure({ props: [U({ DashLine: n.meta })] }) });
function vO(O) {
  let { language: e, support: a } = O.content instanceof b ? O.content : { language: O.content, support: [] };
  return new b(
    xO.configure({
      wrap: E((r) =>
        r.name == "FrontmatterContent" ? { parser: L.parser } : r.name == "Body" ? { parser: e.parser } : null
      ),
    }),
    a
  );
}
export { hO as yaml, vO as yamlFrontmatter, L as yamlLanguage };
