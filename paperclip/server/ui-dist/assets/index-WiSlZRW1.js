import { E as d, g, a as v, p as m, e as u, L as x, s as G, t as a, i as b, h as Z, f as R } from "./index-hX5IdXBW.js";
import { html as h } from "./index-Ddti8Upo.js";
import { L as k, E as f, a as w } from "./index-CW2e1zPo.js";
...
const E = new f((O) => {
  ...
  for (;;) {
    ...
    if ((O.advance(), t == 10)) break;
  }
  O.pos > i && O.acceptToken(U);
});
function _(O, i, t) {
  return new f((r) => {
    let e = r.pos;
    for (;;) {
      ...
      if ((r.advance(), P == 10)) break;
    }
    r.pos > e && r.acceptToken(i);
  });
}
const W = _("endraw", j, z),
  V = {
    __proto__: null,
    in: 38,
    is: 40,
...
const N = S(
  ...
  C = S(
    "boolean callable defined divisibleby eq escaped even filter float ge gt in integer iterable le lower lt mapping ne none number odd sameas sequence string test undefined upper range lipsum dict joiner namespace",
    "function"
  ),
  D = S("loop super self true false varargs kwargs caller name arguments catch_kwargs catch_varargs caller", "keyword"),
  l = C.concat(D),
  $ = S(
    "raw endraw filter endfilter trans pluralize endtrans with endwith autoescape endautoescape if elif else endif for endfor call endcall block endblock set endset macro endmacro import include break continue debug do extends",
    "keyword"
  );
function A(O) {
  var i;
  let { state: t, pos: r } = O,
    e = u(t).resolveInner(r, -1).enterUnfinishedNodesBefore(r),
    P = ((i = e.childBefore(r)) === null || i === void 0 ? void 0 : i.name) || e.name;
  if (e.name == "FilterName") return { type: "filter", node: e };
  if (O.explicit && (P == "FilterOp" || P == "filter")) return { type: "filter" };
  if (e.name == "TagName") return { type: "tag", node: e };
  if (O.explicit && P == "{%") return { type: "tag" };
  if (e.name == "PropertyName" && e.parent.name == "MemberExpression")
    return { type: "prop", node: e, target: e.parent };
  if (e.name == "." && e.parent.name == "MemberExpression") return { type: "prop", target: e.parent };
  if (e.name == "MemberExpression" && P == ".") return { type: "prop", target: e };
  if (e.name == "VariableName") return { type: "expr", from: e.from };
  if (e.name == "Comment" || e.name == "StringLiteral" || e.name == "NumberLiteral") return null;
  let Q = O.matchBefore(/[\w\u00c0-\uffff]+$/);
  return Q ? { type: "expr", from: Q.from } : O.explicit ? { type: "expr" } : null;
}
function L(O, i, t, r) {
  let e = [];
  for (;;) {
    let P = i.getChild("Expression");
    if (!P) return [];
    if (P.name == "VariableName") {
      e.unshift(O.sliceDoc(P.from, P.to));
      break;
    } else if (P.name == "MemberExpression") {
      let Q = P.getChild("PropertyName");
      (Q && e.unshift(O.sliceDoc(Q.from, Q.to)), (i = P));
    } else return [];
  }
  return r(e, O, t);
}
function I(O = {}) {
  let i = O.tags ? O.tags.concat($) : $,
    t = O.variables ? O.variables.concat(l) : l,
    { properties: r } = O;
  return (e) => {
    var P;
    let Q = A(e);
    if (!Q) return null;
    let o = (P = Q.from) !== null && P !== void 0 ? P : Q.node ? Q.node.from : e.pos,
      n;
    return (
      Q.type == "filter"
        ? (n = N)
        : Q.type == "tag"
          ...
          : Q.type == "expr"
            ? (n = t)
            : (n = r ? L(e.state, Q.target, e, r) : []),
      n.length ? { options: n, from: o, validFor: /^[\w\u00c0-\uffff]*$/ } : null
    );
  };
}
const H = d.inputHandler.of((O, i, t, r) =>
  r != "%" || i != t || O.state.doc.sliceString(i - 1, t + 1) != "{}"
    ? !1
    : (O.dispatch(
        O.state.changeByRange((e) => ({
          changes: { from: e.from, to: e.to, insert: "%%" },
          range: g.cursor(e.from + 1),
        })),
...
function p(O) {
  return (i) => {
    let t = O.test(i.textAfter);
    return i.lineIndent(i.node.from) + (t ? 0 : i.unit);
  };
}
const B = x.define({
    name: "jinja",
    parser: F.configure({
      props: [
        G({
          "TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape":
            a.keyword,
          "required scoped recursive with without context ignore missing": a.modifier,
          self: a.self,
          "loop super": a.standard(a.variableName),
          "if elif else endif for endfor call endcall": a.controlKeyword,
          "block endblock set endset macro endmacro import from include": a.definitionKeyword,
          "Comment/...": a.blockComment,
          VariableName: a.variableName,
          Definition: a.definition(a.variableName),
          PropertyName: a.propertyName,
          FilterName: a.special(a.variableName),
          ArithOp: a.arithmeticOperator,
          AssignOp: a.definitionOperator,
          "not and or": a.logicOperator,
          CompareOp: a.compareOperator,
          "in is": a.operatorKeyword,
          "FilterOp ConcatOp": a.operator,
        ...
        b.add({
          Tag: Z({ closing: "%}" }),
          "IfStatement ForStatement": p(/^\s*(\{%-?\s*)?(endif|endfor|else|elif)\b/),
          Statement: p(/^\s*(\{%-?\s*)?end\w/),
        }),
        R.add({
          "Statement Comment"(O) {
            let i = O.firstChild,
              t = O.lastChild;
            return !i || (i.name != "Tag" && i.name != "{#")
              ? null
              : { from: i.to, to: t.name == "EndTag" || t.name == "#}" ? t.from : O.to };
          },
        }),
      ],
    }),
    languageData: { indentOnInput: /^\s*{%-?\s*(?:end|elif|else)$/ },
  }),
  s = h();
function c(O) {
  return B.configure(
    {
      wrap: m((i) =>
        i.type.isTop ? { parser: O.parser, overlay: (t) => t.name == "Text" || t.name == "RawText" } : null
      ),
    },
    "jinja"
  );
}
const J = c(s.language);
function tO(O = {}) {
  let i = O.base || s,
    t = i.language == s.language ? J : c(i.language);
  return new v(t, [
    i.support,
    t.data.of({ autocomplete: I(O) }),
    i.language.data.of({ closeBrackets: { brackets: ["{"] } }),
    H,
  ]);
}
export { H as closePercentBrace, tO as jinja, I as jinjaCompletionSource, J as jinjaLanguage };
