import { L as k, E as $, a as z } from "./index-CUFCPqXX.js";
import { s as R, t, a as Y, L as W, i as Z, c as x, f as G, j as _, e as V, o as E, I as U } from "./index-Cwp91QXi.js";
const T = 135,
  m = 1,
  q = 136,
  C = 137,
...
function S(e) {
  return j(e) || (e >= 97 && e <= 102) || (e >= 65 && e <= 70);
}
const v = (e, r, i) => (O, a) => {
    for (let o = !1, l = 0, n = 0; ; n++) {
      let { next: s } = O;
      if (Q(s) || s == c || s == b || (o && j(s)))
        (!o && (s != c || n > 0) && (o = !0), l === n && s == c && l++, O.advance());
      else if (s == M && O.peek(1) != ee) {
        if ((O.advance(), S(O.next))) {
          do O.advance();
          while (S(O.next));
          O.next == 32 && O.advance();
        ...
        o && O.acceptToken(l == 2 && a.canShift(P) ? r : s == B ? i : e);
        break;
      }
    }
  },
  ae = new $(v(q, P, C), { contextual: !0 }),
  re = new $(v(N, F, I), { contextual: !0 }),
  te = new $((e) => {
    if (X.includes(e.peek(-1))) {
      let { next: r } = e;
      (Q(r) || r == b || r == A || r == L || r == Oe || r == K || (r == D && Q(e.peek(1))) || r == c || r == H) &&
        e.acceptToken(T);
    }
  }),
  oe = new $((e) => {
    if (!X.includes(e.peek(-1))) {
      let { next: r } = e;
      if ((r == J && (e.advance(), e.acceptToken(m)), Q(r))) {
        do e.advance();
        while (Q(e.next) || j(e.next));
        e.acceptToken(m);
      }
    }
  }),
  ...
  ne = { __proto__: null, or: 104, and: 104, not: 112, only: 112, layer: 186 },
  se = { __proto__: null, selector: 118, layer: 182 },
  de = {
    __proto__: null,
    "@import": 178,
    "@media": 190,
  ...
  ce = k.deserialize({
    ...
    tokenizers: [te, oe, ae, re, 1, 2, 3, 4, new z("m~RRYZ[z{a~~g~aO$T~~dP!P!Qg~lO$U~~", 28, 142)],
    topRules: { StyleSheet: [0, 6], Styles: [1, 116] },
    dynamicPrecedences: { 84: 1 },
    specialized: [
      { term: 137, get: (e) => ie[e] || -1 },
      { term: 138, get: (e) => ne[e] || -1 },
      { term: 4, get: (e) => se[e] || -1 },
      { term: 28, get: (e) => de[e] || -1 },
      { term: 136, get: (e) => Qe[e] || -1 },
    ],
    tokenPrec: 2256,
  });
let p = null;
  ...
  if (!p && typeof document == "object" && document.body) {
    ...
    for (let O in e)
      O != "cssText" &&
        O != "cssFloat" &&
        typeof e[O] == "string" &&
        (/[A-Z]/.test(O) && (O = O.replace(/[A-Z]/g, (a) => "-" + a.toLowerCase())), i.has(O) || (r.push(O), i.add(O)));
    p = r.sort().map((O) => ({ type: "property", label: O, apply: O + ": " }));
  }
  return p || [];
}
const g = [
...
function je(e, r) {
  ...
  return (O == null ? void 0 : O.name) != "Callee" ? !1 : r.sliceString(O.from, O.to) == "var";
}
const h = new E(),
  me = ["Declaration"];
function Se(e) {
  for (let r = e; ; ) {
    if (r.type.isTop) return r;
    if (!(r = r.parent)) return e;
  }
}
function w(e, r, i) {
  if (r.to - r.from > 4096) {
    ...
    let a = [],
      o = new Set(),
      l = r.cursor(U.IncludeAnonymous);
    if (l.firstChild())
      do for (let n of w(e, l.node, i)) o.has(n.label) || (o.add(n.label), a.push(n));
      while (l.nextSibling());
    return (h.set(r, a), a);
  } else {
    let O = [],
    ...
    return (
      r.cursor().iterate((o) => {
        ...
        if (
          i(o) &&
          o.matchContext(me) &&
          ((l = o.node.nextSibling) === null || l === void 0 ? void 0 : l.name) == ":"
        ) {
          let n = e.sliceString(o.from, o.to);
          a.has(n) || (a.add(n), O.push({ label: n, type: "variable" }));
        }
      }),
      O
    );
...
const ge = (e) => (r) => {
    let { state: i, pos: O } = r,
      a = V(i).resolveInner(O, -1),
      o = a.type.isError && a.from == a.to - 1 && i.doc.sliceString(a.from, a.to) == "-";
    if (a.name == "PropertyName" || ((o || a.name == "TagName") && /^(Block|Styles)$/.test(a.resolve(a.to).name)))
      return { from: a.from, options: u(), validFor: d };
    if (a.name == "ValueName") return { from: a.from, options: y, validFor: d };
    if (a.name == "PseudoClassName") return { from: a.from, options: g, validFor: d };
    if (e(a) || ((r.explicit || o) && je(a, i.doc)))
      return { from: e(a) || o ? a.from : O, options: w(i.doc, Se(a), e), validFor: ue };
    if (a.name == "TagName") {
      for (let { parent: s } = a; s; s = s.parent)
        if (s.name == "Block") return { from: a.from, options: u(), validFor: d };
      return { from: a.from, options: $e, validFor: d };
    }
    if (a.name == "AtKeyword") return { from: a.from, options: pe, validFor: d };
    if (!r.explicit) return null;
    let l = a.resolve(O),
      n = l.childBefore(O);
    return n && n.name == ":" && l.name == "PseudoClassSelector"
      ? { from: O, options: g, validFor: d }
      : (n && n.name == ":" && l.name == "Declaration") || l.name == "ArgList"
        ? { from: O, options: y, validFor: d }
        : l.name == "Block" || l.name == "Styles"
          ? { from: O, options: u(), validFor: d }
          : null;
  },
  ye = ge((e) => e.name == "VariableName"),
  f = W.define({
    name: "css",
    parser: ce.configure({ props: [Z.add({ Declaration: x() }), G.add({ "Block KeyframeList": _ })] }),
    languageData: { commentTokens: { block: { open: "/*", close: "*/" } }, indentOnInput: /^\s*\}$/, wordChars: "-" },
  });
function Pe() {
  return new Y(f, f.data.of({ autocomplete: ye }));
}
export { Pe as css, ye as cssCompletionSource, f as cssLanguage, ge as defineCSSCompletionSource };
