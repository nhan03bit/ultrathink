import { P as L, N as $, k as E, D as j, l as y, T as S, I as G } from "./index-Cwp91QXi.js";
var N = {};
class v {
  ...
  static start(t, e, s = 0) {
    ...
    return new v(t, [], e, s, s, 0, [], 0, i ? new R(i, i.start) : null, 0, null);
  }
  get context() {
  ...
  reduce(t) {
    ...
    let s = t >> 19,
      ...
      n = h.dynamicPrecedence(i);
    if ((n && (this.score += n), s == 0)) {
      (i < h.minRepeatTerm && this.reducePos < this.pos && (this.reducePos = this.pos),
        this.pushState(h.getGoto(this.state, i, !0), this.reducePos),
        i < h.minRepeatTerm && this.storeNode(i, this.reducePos, this.reducePos, r ? 8 : 4, !0),
        this.reduceContext(i, this.reducePos));
      return;
    }
    let o = this.stack.length - (s - 1) * 3 - (t & 262144 ? 6 : 0),
    ...
    u >= 2e3 &&
      !(!((e = this.p.parser.nodeSet.types[i]) === null || e === void 0) && e.isAnonymous) &&
      (a == this.p.lastBigReductionStart
        ? (this.p.bigReductionCount++, (this.p.lastBigReductionSize = u))
        : this.p.lastBigReductionSize < u &&
          ((this.p.bigReductionCount = 1), (this.p.lastBigReductionStart = a), (this.p.lastBigReductionSize = u)));
    let f = o ? this.stack[o - 1] : 0,
      d = this.bufferBase + this.buffer.length - f;
    if (i < h.minRepeatTerm || t & 131072) {
      let c = h.stateFlag(this.state, 1) ? this.pos : this.reducePos;
  ...
  shift(t, e, s, i) {
    if (t & 131072) this.pushState(t & 65535, this.pos);
      ...
      let h = t,
        { parser: r } = this.p;
      this.pos = i;
      let n = r.stateFlag(h, 1);
      (!n && (i > s || e <= r.maxNode) && (this.reducePos = i),
        this.pushState(h, n ? s : Math.min(s, this.reducePos)),
        this.shiftContext(e, s),
        e <= r.maxNode && this.buffer.push(e, s, i, 4));
    } else ((this.pos = i), this.shiftContext(e, s), e <= this.p.parser.maxNode && this.buffer.push(e, s, i, 4));
  }
  apply(t, e, s, i) {
  ...
  split() {
    let t = this,
      e = t.buffer.length;
    for (e && t.buffer[e - 4] == 0 && (e -= 4); e > 0 && t.buffer[e - 2] > t.reducePos; ) e -= 4;
            ...
            t.allActions(i, (r) => {
              if (!(r & 393216))
                if (r & 65536) {
                  ...
                  if (n > 1) {
                    let o = r & 65535,
                      a = this.stack.length - n * 3;
                    if (a >= 0 && t.getGoto(this.stack[a], o, !1) >= 0) return (n << 19) | 65536 | o;
                  }
                } else {
                  let n = s(r, h + 1);
                  if (n != null) return n;
...
class H {
  ...
  peek(t) {
    ...
    if (e >= 0 && e < this.chunk.length) ((s = this.pos + t), (i = this.chunk.charCodeAt(e)));
      ...
      if (((s = h), s >= this.chunk2Pos && s < this.chunk2Pos + this.chunk2.length))
        ...
        let r = this.rangeIndex,
          n = this.range;
        for (; n.to <= s; ) n = this.ranges[++r];
        ((this.chunk2 = this.input.chunk((this.chunk2Pos = s))),
          s + this.chunk2.length > n.to && (this.chunk2 = this.chunk2.slice(0, n.to - s)),
  ...
  reset(t, e) {
    if (
      (e ? ((this.token = e), (e.start = t), (e.lookAhead = t + 1), (e.value = e.extended = -1)) : (this.token = z),
      this.pos != t)
    ) {
...
class W {
  ...
  token(t, e) {
    ...
    for (;;) {
      ...
      if ((O(this.data, t, e, 0, this.data, this.precTable), t.token.value > -1)) break;
...
W.prototype.contextual = m.prototype.fallback = m.prototype.extend = !1;
class Z {
  constructor(t, e = {}) {
...
function O(l, t, e, s, i, h) {
  let r = 0,
    n = 1 << s,
    { dialect: o } = e.p.parser;
  t: for (; (n & l[r]) != 0; ) {
    let a = l[r + 1];
    for (let c = r + 3; c < a; c += 2)
      if ((l[c + 1] & n) > 0) {
        let p = l[c];
        if (o.allows(p) && (t.token.value == -1 || t.token.value == p || q(p, t.token.value, i, h))) {
          t.acceptToken(p);
          break;
        }
      }
...
function I(l, t, e) {
  for (let s = t, i; (i = l[s]) != 65535; s++) if (i == e) return s - t;
  return -1;
}
function q(l, t, e, s) {
  let i = I(e, s, t);
  return i < 0 || I(e, s, l) < i;
}
const k = typeof process < "u" && N && /\bparse\b/.test(N.LOG);
let T = null;
function D(l, t, e) {
    ...
    if (!(e < 0 ? s.childBefore(t) : s.childAfter(t)))
      for (;;) {
        if ((e < 0 ? s.to < t : s.from > t) && !s.type.isError)
          return e < 0 ? Math.max(0, Math.min(s.to - 1, t - 25)) : Math.min(l.length, Math.max(s.from + 1, t + 25));
        if (e < 0 ? s.prevSibling() : s.nextSibling()) break;
        if (!s.parent()) return e < 0 ? 0 : l.length;
      }
}
class J {
  constructor(t, e) {
  ...
  nextFragment() {
    ...
    if (t) {
      for (
        this.safeFrom = t.openStart ? D(t.tree, t.from + t.offset, 1) - t.offset : t.from,
          this.safeTo = t.openEnd ? D(t.tree, t.to + t.offset, -1) - t.offset : t.to;
        this.trees.length;
      )
      ...
      if (h instanceof S) {
        if (r == t) {
          ...
          if (n <= this.safeTo) {
            ...
            if (!o || n + o < this.fragment.to) return h;
          }
        }
        (this.index[e]++,
          r + h.length >= Math.max(this.safeFrom, t) && (this.trees.push(h), this.start.push(r), this.index.push(0)));
...
class K {
  constructor(t, e) {
    ((this.stream = e),
      ...
      (this.mainToken = null),
      (this.actions = []),
      (this.tokens = t.tokenizers.map((s) => new P())));
  }
  getActions(t) {
    let e = 0,
      s = null,
    ...
    for (let a = 0; a < h.length; a++) {
      ...
      if (
        !(s && !u.fallback) &&
        ((u.contextual || f.start != t.pos || f.mask != r || f.context != n) &&
          (this.updateCachedToken(f, u, t), (f.mask = r), (f.context = n)),
        f.lookAhead > f.end + 25 && (o = Math.max(f.lookAhead, o)),
        f.value != 0)
      ) {
        let d = e;
        if (
  ...
  addActions(t, e, s, i) {
    let { state: h } = t,
      ...
      { data: n } = r;
    for (let o = 0; o < 2; o++)
      for (let a = r.stateSlot(h, o ? 2 : 1); ; a += 3) {
        if (n[a] == 65535)
          if (n[a + 1] == 1) a = g(n, a + 2);
          else {
...
class Q {
  constructor(t, e, s, i) {
    ((this.parser = t),
      ...
      (this.stream = new H(e, i)),
      (this.tokens = new K(t, this.stream)),
      (this.topTerm = t.top[1]));
    let { from: h } = i[0];
    ((this.stacks = [v.start(this, t.top[0], h)]),
      (this.fragments = s.length && this.stream.end - h > t.bufferLength * 4 ? new J(s, t.nodeSet) : null));
  }
  get parsedPos() {
  ...
  advance() {
    ...
    if (!s.length) {
      ...
      if (this.parser.strict) throw new SyntaxError("No parse at " + e);
    ...
    if (this.recovering) {
      ...
      if (s.length > r) for (s.sort((n, o) => o.score - n.score); s.length > r; ) s.pop();
      ...
      t: for (let r = 0; r < s.length - 1; r++) {
        ...
        for (let o = r + 1; o < s.length; o++) {
          ...
          if (n.sameState(a) || (n.buffer.length > 500 && a.buffer.length > 500))
            if ((n.score - a.score || n.buffer.length - a.buffer.length) > 0) s.splice(o--, 1);
            else {
              s.splice(r--, 1);
              continue t;
            }
      ...
      s.length > 12 && (s.sort((r, n) => n.score - r.score), s.splice(12, s.length - 12));
    }
    this.minStackPos = s[0].pos;
    for (let r = 1; r < s.length; r++) s[r].pos < this.minStackPos && (this.minStackPos = s[r].pos);
    return null;
  }
  stopAt(t) {
  ...
  advanceStack(t, e, s) {
    ...
    if (this.fragments) {
      let a = t.curContext && t.curContext.tracker.strict,
        u = a ? t.curContext.hash : 0;
      for (let f = this.fragments.nodeAt(i); f; ) {
        let d = this.parser.nodeSet.types[f.type.id] == f.type ? h.getGoto(t.state, f.type.id) : -1;
        if (d > -1 && f.length && (!a || (f.prop(y.contextHash) || 0) == u)) return (t.useNode(f, d), !0);
        if (!(f instanceof S) || f.children.length == 0 || f.positions[0] > 0) break;
        let c = f.children[0];
        if (c instanceof S && f.positions[0] == 0) f = c;
        else break;
      }
    }
    let n = h.stateSlot(t.state, 4);
    if (n > 0) return (t.reduce(n), !0);
    ...
    for (let a = 0; a < o.length; ) {
      let u = o[a++],
        ...
        c = a == o.length || !s,
        p = c ? t : t.split(),
        b = this.tokens.mainToken;
      if ((p.apply(u, f, b ? b.start : p.pos, d), c)) return !0;
  ...
  runRecovery(t, e, s) {
    ...
    for (let r = 0; r < t.length; r++) {
      let n = t[r],
        ...
        a = e[(r << 1) + 1],
        u = k ? this.stackID(n) + " -> " : "";
      if (n.deadEnd && (h || ((h = !0), n.restart(), this.advanceFully(n, s)))) continue;
      let f = n.split(),
        d = u;
      for (let c = 0; c < 10 && f.forceReduce() && !this.advanceFully(f, s); c++) k && (d = this.stackID(f) + " -> ");
      for (let c of n.recoverByInsert(o)) this.advanceFully(c, s);
      this.stream.end > n.pos
        ? (a == n.pos && (a++, (o = 0)), n.recoverByDelete(o, a), B(n, s))
        : (!i || i.score < f.score) && (i = f);
    }
    return i;
  }
  stackToTree(t) {
...
class _ {
  constructor(t) {
    ((this.start = t.start),
      (this.shift = t.shift || C),
      (this.reduce = t.reduce || C),
      (this.reuse = t.reuse || C),
      (this.hash = t.hash || (() => 0)),
...
class w extends L {
  constructor(t) {
    ...
    let s = Object.keys(t.topRules).map((n) => t.topRules[n][1]),
      i = [];
    for (let n = 0; n < e.length; n++) i.push([]);
    function h(n, o, a) {
    ...
    for (let n = 0; n < this.specializerSpecs.length; n++) this.specialized[n] = this.specializerSpecs[n].term;
    ((this.specializers = this.specializerSpecs.map(F)),
      (this.states = x(t.states, Uint32Array)),
      (this.data = x(t.stateData)),
      (this.goto = x(t.goto)),
      (this.maxTerm = t.maxTerm),
      (this.tokenizers = t.tokenizers.map((n) => (typeof n == "number" ? new m(r, n) : n))),
  ...
  getGoto(t, e, s = !1) {
    ...
    for (let h = i[e + 1]; ; ) {
      let r = i[h++],
        ...
        o = i[h++];
      if (n && s) return o;
      for (let a = h + (r >> 1); h < a; h++) if (i[h] == t) return o;
      if (n) return -1;
    }
  }
  hasAction(t, e) {
    let s = this.data;
    for (let i = 0; i < 2; i++)
      for (let h = this.stateSlot(t, i ? 2 : 1), r; ; h += 3) {
        if ((r = s[h]) == 65535)
          if (s[h + 1] == 1) r = s[(h = g(s, h + 2))];
          else {
            if (s[h + 1] == 2) return g(s, h + 2);
            break;
          }
        if (r == e || r == 0) return g(s, h + 1);
      }
    return 0;
  }
  stateSlot(t, e) {
  ...
  allActions(t, e) {
    let s = this.stateSlot(t, 4),
      i = s ? e(s) : void 0;
    for (let h = this.stateSlot(t, 1); i == null; h += 3) {
      if (this.data[h] == 65535)
        if (this.data[h + 1] == 1) h = g(this.data, h + 2);
        else break;
      i = e(g(this.data, h + 1));
    }
    return i;
  ...
  nextStates(t) {
    ...
    for (let s = this.stateSlot(t, 1); ; s += 3) {
      if (this.data[s] == 65535)
        if (this.data[s + 1] == 1) s = g(this.data, s + 2);
        else break;
...
export { _ as C, Z as E, w as L, W as a };
