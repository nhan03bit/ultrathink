...
function r(n, e) {
  var o = n.sol(),
    i = n.next();
  if (((c = null), o)) {
    if (i == "/") return (e.tokenize = s)(n, e);
    if (i == "\\")
      return n.eol() || /\s/.test(n.peek())
        ? (n.skipToEnd(), /^\\\s*$/.test(n.current()) ? (e.tokenize = v)(n) : (e.tokenize = r), "comment")
        : ((e.tokenize = r), "builtin");
  }
  if (/\s/.test(i)) return n.peek() == "/" ? (n.skipToEnd(), "comment") : "null";
  if (i == '"') return (e.tokenize = m)(n, e);
  if (i == "`") return (n.eatWhile(/[A-Za-z\d_:\/.]/), "macroName");
  if ((i == "." && /\d/.test(n.peek())) || /\d/.test(i)) {
    var t = null;
    return (
      n.backUp(1),
      n.match(/^\d{4}\.\d{2}(m|\.\d{2}([DT](\d{2}(:\d{2}(:\d{2}(\.\d{1,9})?)?)?)?)?)/) ||
      n.match(/^\d+D(\d{2}(:\d{2}(:\d{2}(\.\d{1,9})?)?)?)/) ||
      n.match(/^\d{2}:\d{2}(:\d{2}(\.\d{1,9})?)?/) ||
      n.match(/^\d+[ptuv]{1}/)
        ? (t = "temporal")
        : (n.match(/^0[NwW]{1}/) ||
            n.match(/^0x[\da-fA-F]*/) ||
            ...
            n.match(/-?\d*(\.\d*)?(e[+\-]?\d+)?(e|f)?/)) &&
          (t = "number"),
      t && (!(i = n.peek()) || f.test(i)) ? t : (n.next(), "error")
    );
  }
  return /[A-Za-z]|\./.test(i)
    ? (n.eatWhile(/[A-Za-z._\d]/), p.test(n.current()) ? "keyword" : "variable")
    : /[|/&^!+:\\\-*%$=~#;@><\.,?_\']/.test(i) || /[{}\(\[\]\)]/.test(i)
      ? null
      : "error";
}
...
const a = {
  name: "q",
  startState: function () {
    return { tokenize: r, context: null, indent: 0, col: 0 };
  },
  token: function (n, e) {
    n.sol() && (e.context && e.context.align == null && (e.context.align = !1), (e.indent = n.indentation()));
    var o = e.tokenize(n, e);
    if (
      (o != "comment" && e.context && e.context.align == null && e.context.type != "pattern" && (e.context.align = !0),
      c == "(")
    )
      l(e, ")", n.column());
      ...
      c == "." && e.context && e.context.type == "pattern"
        ? u(e)
        : /atom|string|variable/.test(o) &&
          e.context &&
          (/[\}\]]/.test(e.context.type)
            ? l(e, "pattern", n.column())
  ...
  indent: function (n, e, o) {
    var i = e && e.charAt(0),
      t = n.context;
    if (/[\]\}]/.test(i)) for (; t && t.type == "pattern"; ) t = t.prev;
    var d = t && i == t.type;
