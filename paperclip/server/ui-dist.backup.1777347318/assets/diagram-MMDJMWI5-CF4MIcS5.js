...
import { p as H } from "./chunk-4BX2VUAB-Cp4uHV7g.js";
import { p as V } from "./wardley-RL74JXVD-BZ2WBREQ.js";
...
var x = { showLegend: !0, ticks: 5, max: null, min: 0, graticule: "circle" },
  w = { axes: [], curves: [], options: x },
  m = structuredClone(w),
  j = G.radar,
  q = l(() => y({ ...j, ...C().radar }), "getConfig"),
  b = l(() => m.axes, "getAxes"),
  N = l(() => m.curves, "getCurves"),
  U = l(() => m.options, "getOptions"),
  X = l((a) => {
    m.axes = a.map((t) => ({ name: t.name, label: t.label ?? t.name }));
  }, "setAxes"),
  Y = l((a) => {
    m.curves = a.map((t) => ({ name: t.name, label: t.label ?? t.name, entries: Z(t.entries) }));
  }, "setCurves"),
  Z = l((a) => {
  ...
  J = l((a) => {
    var e, r, s, o, i;
    const t = a.reduce((n, c) => ((n[c.name] = c), n), {});
    m.options = {
  ...
  et = l((a, t, e, r) => {
    const s = r.db,
      ...
      p = D(t),
      u = at(p, c),
      g = n.max ?? Math.max(...i.map((f) => Math.max(...f.entries))),
      h = n.min,
      v = Math.min(c.width, c.height) / 2;
    (rt(u, o, v, n.ticks, n.graticule),
      st(u, o, v, c),
      M(u, o, i, h, g, n.graticule, c),
      T(u, i, n.showLegend, c),
      u
        .append("text")
  ...
  at = l((a, t) => {
    ...
    return (
      B(a, r, e, t.useMaxWidth ?? !0),
      a.attr("viewBox", `0 0 ${e} ${r}`),
  ...
  st = l((a, t, e, r) => {
    ...
    for (let o = 0; o < s; o++) {
      const i = t[o].label,
        n = (2 * o * Math.PI) / s - Math.PI / 2;
      (a
        .append("line")
        .attr("x1", 0)
        .attr("y1", 0)
        .attr("x2", e * r.axisScaleFactor * Math.cos(n))
        .attr("y2", e * r.axisScaleFactor * Math.sin(n))
        .attr("class", "radarAxisLine"),
        a
          .append("text")
          .text(i)
          .attr("x", e * r.axisLabelFactor * Math.cos(n))
          .attr("y", e * r.axisLabelFactor * Math.sin(n))
...
function M(a, t, e, r, s, o, i) {
  ...
  e.forEach((d, p) => {
    if (d.entries.length !== n) return;
    const u = d.entries.map((g, h) => {
      const v = (2 * Math.PI * h) / n - Math.PI / 2,
        f = A(g, r, s, c),
        S = f * Math.cos(v),
        O = f * Math.sin(v);
      return { x: S, y: O };
    ...
    o === "circle"
      ? a.append("path").attr("d", L(u, i.curveTension)).attr("class", `radarCurve-${p}`)
      : o === "polygon" &&
        a
          .append("polygon")
...
function L(a, t) {
  ...
  for (let s = 0; s < e; s++) {
    const o = a[(s - 1 + e) % e],
      ...
      n = a[(s + 1) % e],
      c = a[(s + 2) % e],
      d = { x: i.x + (n.x - o.x) * t, y: i.y + (n.y - o.y) * t },
      p = { x: n.x - (c.x - i.x) * t, y: n.y - (c.y - i.y) * t };
...
var nt = { draw: et },
  ...
  lt = l(({ radar: a } = {}) => {
    const { themeVariables: t, radarOptions: e } = it(a);
    return `
	.radarTitle {
  ...
  xt = { parser: tt, db: $, renderer: nt, styles: lt };
