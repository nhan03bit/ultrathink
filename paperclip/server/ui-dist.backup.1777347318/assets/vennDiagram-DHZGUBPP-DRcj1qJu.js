    ...
    for (let u = 0; u < e.length; ++u) {
      ...
      for (let m = 0; m < a.parentIndex.length; ++m)
        if (f.parentIndex.includes(a.parentIndex[m])) {
          const x = t[a.parentIndex[m]],
            M = Math.atan2(a.x - x.x, a.y - x.y),
            E = Math.atan2(f.x - x.x, f.y - x.y);
          let _ = E - M;
          _ < 0 && (_ += 2 * Math.PI);
          const w = E - _ / 2;
          let g = W(y, { x: x.x + x.radius * Math.sin(w), y: x.y + x.radius * Math.cos(w) });
          (g > x.radius * 2 && (g = x.radius * 2),
            (d == null || d.width > g) && (d = { circle: x, width: g, p1: a, p2: f, large: g > x.radius, sweep: !0 }));
        }
      d != null && (o.push(d), (i += lt(d.circle.radius, d.width)), (f = a));
    }
  } else {
    let c = t[0];
    for (let u = 1; u < t.length; ++u) t[u].radius < c.radius && (c = t[u]);
    let f = !1;
...
function le(t, n, s, e) {
  ...
  const i = e.maxIterations || 100,
    ...
    c = t(s);
  let f = s - n;
  if (o * c > 0) throw "Initial bisect points must have opposite signs";
  if (o === 0) return n;
  if (c === 0) return s;
  for (let u = 0; u < i; ++u) {
    f /= 2;
    const a = n + f,
      y = t(a);
    if ((y * o >= 0 && (n = a), Math.abs(f) < r || y === 0)) return a;
  }
  return n + f;
}
function ct(t) {
...
function zt(t, n, s) {
  ...
  function M(v) {
    for (let l = 0; l < v.length; l++) x[m][l] = v[l];
    x[m].fx = v.fx;
  }
  const E = (v, l) => v.fx - l.fx,
    _ = n.slice(),
    w = n.slice(),
    g = n.slice(),
    b = n.slice();
  for (let v = 0; v < e; ++v) {
    if ((x.sort(E), s.history)) {
      ...
      (p.sort((h, j) => h.id - j.id), s.history.push({ x: x[0].slice(), fx: x[0].fx, simplex: p }));
    }
    d = 0;
    for (let p = 0; p < m; ++p) d = Math.max(d, Math.abs(x[0][p] - x[1][p]));
    if (Math.abs(x[0].fx - x[m].fx) < o && d < c) break;
    for (let p = 0; p < m; ++p) {
      _[p] = 0;
      for (let h = 0; h < m; ++h) _[p] += x[h][p];
      _[p] /= m;
    }
    const l = x[m];
    if ((Q(w, 1 + f, _, -f, l), (w.fx = t(w)), w.fx < x[0].fx))
      ...
      if (
        (w.fx > l.fx
          ? (Q(g, 1 + a, _, -a, l), (g.fx = t(g)), g.fx < l.fx ? M(g) : (p = !0))
          : (Q(g, 1 - a * f, _, a * f, l), (g.fx = t(g)), g.fx < w.fx ? M(g) : (p = !0)),
        p)
      ) {
        if (y >= 1) break;
        for (let h = 1; h < x.length; ++h) (Q(x[h], 1 - y, x[0], y, x[h]), (x[h].fx = t(x[h])));
      }
    } else M(w);
  }
...
function ce(t, n, s, e, i, r, o) {
  ...
  for (let x = 0; x < 10; ++x) {
    if (
      (Q(e.x, 1, s.x, i, n), (u = e.fx = t(e.x, e.fxprime)), (y = tt(e.fxprime, n)), u > c + r * i * f || (x && u >= a))
    )
      return m(d, i, a);
    if (Math.abs(y) <= -o * f) return i;
    if (y >= 0) return m(i, d, u);
    ((a = u), (d = i), (i *= 2));
  }
  return i;
}
function ue(t, n, s) {
  ...
  for (let a = 0; a < u; ++a) {
    if (
      ...
      const y = tt(e.fxprime, e.fxprime),
        d = Math.max(0, tt(r, i.fxprime) / y);
      (Q(o, d, o, -1, i.fxprime), (c = e), (e = i), (i = c));
    }
    if (ut(e.fxprime) <= 1e-5) break;
  }
  return (s.history && s.history.push({ x: e.x.slice(), fx: e.fx, fxprime: e.fxprime.slice(), alpha: f }), e);
}
function At(t, n = {}) {
  n.maxIterations = n.maxIterations || 500;
  const s = n.initialLayout || ge,
    e = n.lossFunction || et,
    i = he(t, n),
    r = s(i, n),
    o = Object.keys(r),
    c = [];
  for (const a of o) (c.push(r[a].x), c.push(r[a].y));
  const u = zt(
    (a) => {
      const y = {};
      for (let d = 0; d < o.length; ++d) {
        const m = o[d];
        y[m] = { x: a[2 * d], y: a[2 * d + 1], radius: r[m].radius };
      }
      return e(y, i);
    },
    c,
...
function he(t, n = {}) {
  ...
  const r = [],
    o = new Set();
  for (const c of e)
    if (c.sets.length === 1) r.push(c.sets[0]);
...
function fe(t, n, s) {
  const e = Mt(n.length, n.length),
    i = Mt(n.length, n.length);
  return (
    t
      ...
      .forEach((r) => {
        const o = s[r.sets[0]],
          c = s[r.sets[1]],
          f = Math.sqrt(n[o].size / Math.PI),
          u = Math.sqrt(n[c].size / Math.PI),
          a = ft(f, u, r.size);
        e[o][c] = e[c][o] = a;
        let y = 0;
        (r.size + 1e-10 >= Math.min(n[o].size, n[c].size) ? (y = 1) : r.size <= 1e-10 && (y = -1),
          (i[o][c] = i[c][o] = y));
      }),
...
function xe(t, n = {}) {
  const s = n.restarts || 10,
    e = [],
    i = {};
  for (const d of t) d.sets.length === 1 && ((i[d.sets[0]] = e.length), e.push(d));
  ...
  let u = null;
  for (let d = 0; d < s; ++d) {
    const m = ct(r.length * 2).map(Math.random),
      x = ue(f, m, n);
    (!u || x.fx < u.fx) && (u = x);
  }
  const a = u.x,
  ...
  for (let d = 0; d < e.length; ++d) {
    const m = e[d];
    y[m.sets[0]] = { x: a[2 * d] * c, y: a[2 * d + 1] * c, radius: Math.sqrt(m.size / Math.PI) };
  }
  if (n.history) for (const d of n.history) ht(d.x, c);
  return y;
}
function ye(t, n) {
  const s = n && n.lossFunction ? n.lossFunction : et,
    e = {},
    i = {};
  for (const y of t)
    if (y.sets.length === 1) {
      const d = y.sets[0];
      ((e[d] = { x: 1e10, y: 1e10, rowid: e.length, size: y.size, radius: Math.sqrt(y.size / Math.PI) }), (i[d] = []));
    }
  ...
  for (const y of t) {
    ...
    const m = y.sets[0],
      x = y.sets[1];
    (y.size + Rt >= Math.min(e[m].size, e[x].size) && (d = 0),
      i[m].push({ set: x, size: y.size, weight: d }),
      i[x].push({ set: m, size: y.size, weight: d }));
  }
  const r = [];
  Object.keys(i).forEach((y) => {
    let d = 0;
    for (let m = 0; m < i[y].length; ++m) d += i[y][m].size * i[y][m].weight;
    r.push({ set: y, size: d });
  });
  function o(y, d) {
  ...
  for (let y = 1; y < r.length; ++y) {
    const d = r[y].set,
      m = i[d].filter(f),
      x = e[d];
    if ((m.sort(o), m.length === 0)) throw "ERROR: missing pairwise overlap information";
    const M = [];
    for (var a = 0; a < m.length; ++a) {
      const w = e[m[a].set],
        g = ft(x.radius, w.radius, m[a].size);
      (M.push({ x: w.x + g, y: w.y }),
        M.push({ x: w.x - g, y: w.y }),
        M.push({ y: w.y + g, x: w.x }),
        M.push({ y: w.y - g, x: w.x }));
      for (let b = a + 1; b < m.length; ++b) {
        const v = e[m[b].set],
          l = ft(x.radius, v.radius, m[b].size),
          p = Tt({ x: w.x, y: w.y, radius: g }, { x: v.x, y: v.y, radius: l });
        M.push(...p);
      }
    }
    let E = 1e50,
...
function pe(t, n, s) {
  ...
  if (
    (t.length === 2 &&
      W(t[0], t[1]) < Math.abs(t[1].radius - t[0].radius) &&
...
function Nt(t, n, s, e, i) {
  ...
  if (o.max === o.min || c.max === c.min) return t;
  let f, u;
  if (i) {
    const m = Math.sqrt(i / Math.PI) * 2;
    ((f = n / m), (u = s / m));
  } else ((f = n / (o.max - o.min)), (u = s / (c.max - c.min)));
  const a = Math.min(u, f),
    y = (n - (o.max - o.min) * a) / 2,
    d = (s - (c.max - c.min) * a) / 2;
  return Ot(
    r.map((m) => ({ radius: a * m.radius, x: e + y + (m.x - o.min) * a, y: e + d + (m.y - c.min) * a, setid: m.setid }))
  );
}
function Ot(t) {
  const n = {};
...
function be(t = {}) {
  let n = !1,
    ...
    m = !1,
    x = null,
    M = t && t.symmetricalTextCentre ? t.symmetricalTextCentre : !1,
    E = {},
    _ =
      t && t.colourScheme
        ? t.colourScheme
  ...
  function l(p) {
    ...
    function B(I) {
      if (I.sets in Y) return Y[I.sets];
      ...
      return (L) => {
        const G = I.sets.map((Z) => {
          let H = q[Z],
            J = k[Z];
          return (
            H || (H = { x: s / 2, y: e / 2, radius: 1 }),
            J || (J = { x: s / 2, y: e / 2, radius: 1 }),
            { x: H.x * (1 - L) + J.x * L, y: H.y * (1 - L) + J.y * L, radius: H.radius * (1 - L) + J.radius * L }
          );
        });
        return wt(G, x);
      };
    }
    const D = U.selectAll(".venn-area").data(h, (I) => I.sets),
      O = D.enter()
        .append("g")
        .attr(
          "class",
      ...
      V = O.append("text")
        ...
        .attr("y", e / 2);
    a &&
      (P.style("fill-opacity", "0")
        .filter((I) => I.sets.length == 1)
        .style("fill", (I) => (I.colour ? I.colour : I.color ? I.color : g(I.sets)))
        .style("fill-opacity", ".25"),
      V.style("fill", (I) =>
        I.colour || I.color ? "#FFF" : t.textFill ? t.textFill : I.sets.length == 1 ? g(I.sets) : "#444"
      ));
    function R(I) {
      return typeof I.transition == "function" ? I.transition("venn").duration(r) : I;
    }
    let T = p;
    z && typeof T.transition == "function"
      ? ((T = R(p)), T.selectAll("path").attrTween("d", A))
      : T.selectAll("path").attr("d", (I) => wt(I.sets.map((L) => k[L])), x);
    const F = T.selectAll("text")
      .filter((I) => I.sets in N)
      .text((I) => B(I))
      .attr("x", (I) => Math.floor(N[I.sets].x))
      .attr("y", (I) => Math.floor(N[I.sets].y));
    ...
    const X = C.selectAll("text")
      .attr("x", s / 2)
      .attr("y", e / 2);
    return (
      y !== null && (V.style("font-size", "0px"), F.style("font-size", y), X.style("font-size", "0px")),
      { circles: k, textCentres: N, nodes: D, enter: O, update: T, exit: C }
    );
  }
  return (
    (l.wrap = function (p) {
...
function ot(t, n) {
  return function (s) {
    ...
    for (; (u = o.pop()), !!u; ) {
      ...
      ((M.textContent = g),
        g.length > f &&
          M.getComputedTextLength() > i &&
          (a.pop(), (M.textContent = a.join(" ")), (a = [u]), (M = x(u)), y++));
    }
    const E = 0.35 - (y * d) / 2,
      _ = e.getAttribute("x"),
      w = e.getAttribute("y");
...
function Lt(t, n, s) {
  ...
  const o = zt((a) => -1 * at({ x: a[0], y: a[1] }, t, n), [i.x, i.y], { maxIterations: 500, minErrorDelta: 1e-10 }).x,
    c = { x: s ? 0 : o[0], y: o[1] };
  let f = !0;
  for (const a of t)
    if (W(c, a) > a.radius) {
...
function jt(t, n, s) {
  ...
  for (let r = 0; r < n.length; ++r) {
    ...
    for (let d = 0; d < o.length; ++d) {
      c[o[d]] = !0;
      const m = i[o[d]];
      for (let x = 0; x < m.length; ++x) f[m[x]] = !0;
    }
    const u = [],
      a = [];
    for (let d in t) d in c ? u.push(t[d]) : d in f || a.push(t[d]);
    const y = Lt(u, a, s);
    ((e[o] = y), y.disjoint && n[r].size > 0);
  }
  return e;
}
function Ie(t, n, s) {
...
function Me(t, n = {}) {
  ...
  let x = e(t, { lossFunction: s === "default" || !s ? et : s === "logRatio" ? Dt : s, distinct: d });
  i && (x = Ct(x, r, o));
  const M = Nt(x, c, f, u, a),
    E = jt(M, t, y),
    _ = new Map(Object.keys(M).map((b) => [b, { set: b, x: M[b].x, y: M[b].y, radius: M[b].radius }])),
    w = t.map((b) => {
      const v = b.sets.map((h) => _.get(h)),
        l = Pt(v),
        p = Vt(l, m);
  ...
  function g(b) {
    let v = "";
    for (const l of w) l.has.size > b.length && b.every((p) => l.has.has(p)) && (v += " " + l.path);
    return v;
  }
...
var gt = (function () {
  var t = S(function (w, g, b, v) {
    ...
    M = {
      ...
      performAction: S(function (g, b, v, l, p, h, j) {
        var k = h.length - 1;
        switch (p) {
          case 1:
            return h[k - 1];
          ...
          case 12:
            ...
            break;
          case 13:
            if (h[k].length < 2) throw new Error("union requires multiple identifiers");
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(h[k]),
              l.addSubsetData(h[k], void 0, void 0),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 14:
            if (h[k - 1].length < 2) throw new Error("union requires multiple identifiers");
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(h[k - 1]),
              l.addSubsetData(h[k - 1], h[k], void 0),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 15:
            if (h[k - 2].length < 2) throw new Error("union requires multiple identifiers");
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(h[k - 2]),
              l.addSubsetData(h[k - 2], void 0, parseFloat(h[k])),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 16:
            if (h[k - 3].length < 2) throw new Error("union requires multiple identifiers");
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(h[k - 3]),
              l.addSubsetData(h[k - 3], h[k - 2], parseFloat(h[k])),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 17:
          case 18:
      ...
      table: [
        ...
        t(s, [2, 21]),
        { 20: c, 26: 51 },
        { 16: u, 20: a, 21: [1, 53], 28: 52, 29: 54, 30: 55, 31: y, 32: d, 33: m },
        t(s, [2, 12]),
        t(s, [2, 16]),
        t(f, [2, 30]),
        t(f, [2, 31]),
        t(f, [2, 32]),
        t(f, [2, 33], { 30: 61, 16: u, 20: a, 31: y, 32: d, 33: m }),
        t(x, [2, 34]),
        t(x, [2, 36]),
        t(x, [2, 37]),
        t(x, [2, 38]),
      ...
      parseError: S(function (g, b) {
        if (b.recoverable) this.trace(g);
        else {
          var v = new Error(g);
          throw ((v.hash = b), v);
        }
      ...
      parse: S(function (g) {
        var b = this,
          ...
          B = 2,
          U = 1,
          q = h.slice.call(arguments, 1),
          z = Object.create(this.lexer),
          A = { yy: {} };
        for (var D in this.yy) Object.prototype.hasOwnProperty.call(this.yy, D) && (A.yy[D] = this.yy[D]);
        (z.setInput(g, A.yy), (A.yy.lexer = z), (A.yy.parser = this), typeof z.yylloc > "u" && (z.yylloc = {}));
        var O = z.yylloc;
        h.push(O);
        var P = z.options && z.options.ranges;
        typeof A.yy.parseError == "function"
          ? (this.parseError = A.yy.parseError)
        ...
        function R() {
          ...
          return (
            (K = l.pop() || z.lex() || U),
            typeof K != "number" && (K instanceof Array && ((l = K), (K = l.pop())), (K = b.symbols_[K] || K)),
            K
          );
        }
        S(R, "lex");
        for (var T, F, C, X, I = {}, L, G, Z, H; ; ) {
          if (
            ((F = v[v.length - 1]),
            this.defaultActions[F]
              ? (C = this.defaultActions[F])
              : ((T === null || typeof T > "u") && (T = R()), (C = j[F] && j[F][T])),
            typeof C > "u" || !C.length || !C[0])
          ) {
            var J = "";
            H = [];
            for (L in j[F]) this.terminals_[L] && L > B && H.push("'" + this.terminals_[L] + "'");
            (z.showPosition
              ? (J =
                  "Parse error on line " +
                  (N + 1) +
...
Expecting ` +
              ...
              : (J =
                  ...
                  (N + 1) +
                  ": Unexpected " +
                  (T == U ? "end of input" : "'" + (this.terminals_[T] || T) + "'")),
              this.parseError(J, {
                text: z.match,
          ...
          switch (C[0]) {
            ...
            case 2:
              if (
                ...
                P && (I._$.range = [h[h.length - (G || 1)].range[0], h[h.length - 1].range[1]]),
                (X = this.performAction.apply(I, [k, Y, N, A.yy, C[1], p, h].concat(q))),
                typeof X < "u")
              )
                return X;
              (G && ((v = v.slice(0, -1 * G * 2)), (p = p.slice(0, -1 * G)), (h = h.slice(0, -1 * G))),
                v.push(this.productions_[C[1]][0]),
                p.push(I.$),
                h.push(I._$),
                (Z = j[v[v.length - 2]][v[v.length - 1]]),
                v.push(Z));
              break;
            case 3:
              return !0;
    ...
    E = (function () {
      var w = {
        EOF: 1,
        parseError: S(function (b, v) {
          if (this.yy.parser) this.yy.parser.parseError(b, v);
          else throw new Error(b);
        }, "parseError"),
        setInput: S(function (g, b) {
          return (
            (this.yy = b || this.yy || {}),
            (this._input = g),
        ...
        unput: S(function (g) {
          ...
          return (
            (this.yylloc = {
              ...
              last_column: v
                ? (v.length === l.length ? this.yylloc.first_column : 0) + l[l.length - v.length].length - v[0].length
                : this.yylloc.first_column - b,
            }),
            this.options.ranges && (this.yylloc.range = [p[0], p[0] + this.yyleng - b]),
...
` +
        ...
        upcomingInput: S(function () {
          var g = this.match;
          return (
            g.length < 20 && (g += this._input.substr(0, 20 - g.length)),
            (g.substr(0, 20) + (g.length > 20 ? "..." : "")).replace(/\n/g, "")
          );
        }, "upcomingInput"),
        showPosition: S(function () {
          var g = this.pastInput(),
            b = new Array(g.length + 1).join("-");
          return (
            g +
            this.upcomingInput() +
            `
` +
        ...
        test_match: S(function (g, b) {
          ...
          if (
            ...
            (this.matched += g[0]),
            (v = this.performAction.call(this, this.yy, this, b, this.conditionStack[this.conditionStack.length - 1])),
            this.done && this._input && (this.done = !1),
            v)
          )
            return v;
          if (this._backtrack) {
            for (var h in p) this[h] = p[h];
            return !1;
          }
          return !1;
        }, "test_match"),
        next: S(function () {
          ...
          for (var p = this._currentRules(), h = 0; h < p.length; h++)
            if (((v = this._input.match(this.rules[p[h]])), v && (!b || v[0].length > b[0].length))) {
              if (((b = v), (l = h), this.options.backtrack_lexer)) {
                if (((g = this.test_match(v, p[h])), g !== !1)) return g;
                if (this._backtrack) {
                  b = !1;
                  continue;
                } else return !1;
          ...
          return b
            ? ((g = this.test_match(b, p[l])), g !== !1 ? g : !1)
            : this._input === ""
              ? this.EOF
              : this.parseError(
                  "Lexical error on line " +
...
` +
        ...
        performAction: S(function (b, v, l, p) {
          switch (l) {
            case 0:
              break;
            case 1:
            ...
            case 2:
              break;
            case 3:
              if (b.getIndentMode && b.getIndentMode()) return ((b.consumeIndentText = !0), this.begin("INITIAL"), 22);
              break;
            case 4:
              break;
            case 5:
            ...
            case 13:
              return 17;
            case 14:
              if (b.consumeIndentText) b.consumeIndentText = !1;
              else return 19;
              break;
            case 15:
              return 24;
...
var we = gt,
  ...
  Se = S((t, n, s) => {
    const e = it(t).sort(),
      i = s ?? 10 / Math.pow(t.length, 2);
    ((vt = e), e.length === 1 && bt.add(e[0]), yt.push({ sets: e, size: i, label: n ? nt(n) : void 0 }));
  }, "addSubsetData"),
  _e = S(() => yt, "getSubsetData"),
  nt = S((t) => {
    const n = t.trim();
    return n.length >= 2 && n.startsWith('"') && n.endsWith('"') ? n.slice(1, -1) : n;
  }, "normalizeText"),
  Te = S((t) => t && nt(t), "normalizeStyleValue"),
  Ee = S((t, n, s) => {
    const e = nt(n);
    pt.push({ sets: it(t).sort(), id: e, label: s ? nt(s) : void 0 });
  }, "addTextData"),
  ze = S((t, n) => {
    const s = it(t).sort(),
      e = {};
    for (const [i, r] of n) e[i] = Te(r) ?? r;
    mt.push({ targets: s, styles: e });
  }, "addStyleData"),
  Ae = S(() => mt, "getStyleData"),
  it = S((t) => t.map((n) => nt(n)), "normalizeIdentifierList"),
  Re = S((t) => {
    const s = it(t).filter((e) => !bt.has(e));
    if (s.length > 0) throw new Error(`unknown set identifier: ${s.join(", ")}`);
  }, "validateUnionIdentifiers"),
  De = S(() => pt, "getTextData"),
  Ce = S(() => vt, "getCurrentSets"),
  Ne = S(() => It, "getIndentMode"),
  Oe = S((t) => {
    It = t;
  }, "setIndentMode"),
  Fe = re.venn;
...
var Be = S((t, n, s, e) => {
  var U, q, z;
  const i = e.db,
    r = (U = i.getConfig) == null ? void 0 : U.call(i),
    { themeVariables: o, look: c, handDrawnSeed: f } = St(),
    u = c === "handDrawn",
    a = [o.venn1, o.venn2, o.venn3, o.venn4, o.venn5, o.venn6, o.venn7, o.venn8].filter(Boolean),
    y = (q = i.getDiagramTitle) == null ? void 0 : q.call(i),
    d = i.getSubsetData(),
    m = i.getTextData(),
    x = qt(i.getStyleData()),
    M = (r == null ? void 0 : r.width) ?? 800,
    E = (r == null ? void 0 : r.height) ?? 450,
    w = M / 1600,
    g = y ? 48 * w : 0,
    b = o.primaryTextColor ?? o.textColor,
    v = Jt(n);
  (v.attr("viewBox", `0 0 ${M} ${E}`),
    y &&
      v
        .append("text")
        .text(y)
        .attr("class", "venn-title")
        .attr("font-size", `${32 * w}px`)
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "middle")
        .attr("x", "50%")
        .attr("y", 32 * w)
        .style("fill", o.vennTitleTextColor || o.titleColor));
  const l = rt(document.createElement("div")),
    p = be()
      .width(M)
      .height(E - g);
  l.datum(d).call(p);
  const h = u ? Qt.svg(l.select("svg").node()) : void 0,
    j = Me(d, { width: M, height: E - g, padding: (r == null ? void 0 : r.padding) ?? 15 }),
    k = new Map();
  for (const A of j) {
  ...
  (l.selectAll(".venn-circle").each(function (A, D) {
    ...
    const O = rt(this),
      V = $([...A.sets].sort()),
      R = x.get(V),
      T = (R == null ? void 0 : R.fill) || a[D % a.length] || o.primaryColor;
    O.classed(`venn-set-${D % 8}`, !0);
    const F = (R == null ? void 0 : R["fill-opacity"]) ?? 0.1,
      C = (R == null ? void 0 : R.stroke) || T,
      X = (R == null ? void 0 : R["stroke-width"]) || `${5 * w}`;
    if (u && h) {
      const G = k.get(V);
      if (G && G.circles.length > 0) {
        const Z = G.circles[0],
          H = h.circle(Z.x, Z.y, Z.radius * 2, {
            roughness: 0.7,
            seed: f,
            fill: kt(T, 0.7),
            fillStyle: "hachure",
    ...
    O.select("text")
      .style("font-size", `${48 * w}px`)
      .style("fill", I);
  }),
    u && h
      ? l.selectAll(".venn-intersection").each(function (A) {
          var T;
          const D = rt(this),
            P = $([...A.sets].sort()),
            V = x.get(P),
            R = V == null ? void 0 : V.fill;
          if (R) {
            const F = D.select("path"),
              C = F.attr("d");
            if (C) {
              const X = h.path(C, {
                  roughness: 0.7,
          ...
          D.select("text")
            .style("font-size", `${48 * w}px`)
            .style("fill", (V == null ? void 0 : V.color) ?? o.vennSetTextColor ?? b);
        })
      : (l
  ...
  const Y = v.append("g").attr("transform", `translate(0, ${g})`),
    B = l.select("svg").node();
  if (B && "childNodes" in B) for (const A of [...B.childNodes]) (z = Y.node()) == null || z.appendChild(A);
...
function Ut(t, n, s, e, i, r) {
  var a;
  const o = (t == null ? void 0 : t.useDebugLayout) ?? !1,
  ...
  for (const [y, d] of u.entries()) {
    const m = n.get(y);
    if (!(m != null && m.text)) continue;
    const x = m.text.x,
      M = m.text.y,
      E = Math.min(...m.circles.map((q) => q.radius)),
      _ = Math.min(...m.circles.map((q) => q.radius - Math.hypot(x - q.x, M - q.y)));
    ...
    const b = Math.max(80 * i, w * 2 * 0.95),
      v = Math.max(60 * i, w * 2 * 0.95),
      h = (m.data.label && m.data.label.length > 0 ? Math.min(32 * i, w * 0.25) : 0) + (d.length <= 2 ? 30 * i : 0),
      j = x - b / 2,
      k = M - v / 2 + h,
      N = Math.max(1, Math.ceil(Math.sqrt(d.length))),
      Y = Math.max(1, Math.ceil(d.length / N)),
      B = b / N,
      U = v / Y;
    for (const [q, z] of d.entries()) {
      ...
      const V = B * 0.9,
        ...
        F = (a = r.get(z.id)) == null ? void 0 : a.color,
        C = T.append("xhtml:span")
...
var qe = { draw: Be },
  We = { parser: we, db: je, renderer: qe, styles: Ve };
