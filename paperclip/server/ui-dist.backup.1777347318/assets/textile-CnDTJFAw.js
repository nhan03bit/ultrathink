...
function h(i, e) {
  ((e.mode = r.newLayout),
    (e.tableHeading = !1),
    e.layoutType === "definitionList" &&
      e.spanningLayout &&
      i.match(l("definitionListEnd"), !1) &&
      (e.spanningLayout = !1));
}
function s(i, e, n) {
  if (n === "_") return i.eat("_") ? a(i, e, "italic", /__/, 2) : a(i, e, "em", /_/, 1);
  if (n === "*") return i.eat("*") ? a(i, e, "bold", /\*\*/, 2) : a(i, e, "strong", /\*/, 1);
  if (n === "[") return (i.match(/\d+\]/) && (e.footCite = !0), u(e));
  if (n === "(") {
    var o = i.match(/^(r|tm|c)\)/);
    if (o) return f.specialChar;
  }
  if (n === "<" && i.match(/(\w+)[^>]+>[^<]+<\/\1>/)) return f.html;
  if (n === "?" && i.eat("?")) return a(i, e, "cite", /\?\?/, 2);
  if (n === "=" && i.eat("=")) return a(i, e, "notextile", /==/, 2);
  if (n === "-" && !i.eat("-")) return a(i, e, "deletion", /-/, 1);
  if (n === "+") return a(i, e, "addition", /\+/, 1);
  if (n === "~") return a(i, e, "sub", /~/, 1);
  if (n === "^") return a(i, e, "sup", /\^/, 1);
  if (n === "%") return a(i, e, "span", /%/, 1);
  if (n === "@") return a(i, e, "code", /@/, 1);
  if (n === "!") {
    var c = a(i, e, "image", /(?:\([^\)]+\))?!/, 1);
    return (i.match(/^:\S+/), c);
  }
...
function a(i, e, n, o, c) {
  var d = i.pos > c ? i.string.charAt(i.pos - c - 1) : null,
    p = i.peek();
  if (e[n]) {
    ...
    (!d || /\W/.test(d)) &&
      p &&
      /\S/.test(p) &&
      i.match(new RegExp("^.*\\S" + o.source + "(?:\\W|$)"), !1) &&
      ((e[n] = !0), (e.mode = r.attributes));
  return u(e);
}
function u(i) {
  var e = b(i);
  ...
  var n = [];
  return (
    i.layoutType && n.push(f[i.layoutType]),
    (n = n.concat(
      g(
        i,
        "addition",
    ...
    i.layoutType === "header" && n.push(f.header + "-" + i.header),
...
function g(i) {
  for (var e = [], n = 1; n < arguments.length; ++n) i[arguments[n]] && e.push(f[arguments[n]]);
  return e;
}
function m(i) {
  var e = i.spanningLayout,
    n = i.layoutType;
...
var t = {
  ...
  createRe: function (i) {
    switch (i) {
      ...
      case "tableCellAttributes":
        return t.makeRe("^", t.choiceRe(t.single.tableCellAttributes, l("allAttributes")), "+\\.");
      case "type":
        return t.makeRe("^", l("allTypes"));
      case "typeLayout":
        return t.makeRe("^", l("allTypes"), l("allAttributes"), "*\\.\\.?", "(\\s+|$)");
      case "attributes":
  ...
  choiceRe: function () {
    for (var i = [arguments[0]], e = 1; e < arguments.length; ++e) ((i[e * 2 - 1] = "|"), (i[e * 2] = arguments[e]));
...
var r = {
  newLayout: function (i, e) {
    if (i.match(l("typeLayout"), !1)) return ((e.spanningLayout = !1), (e.mode = r.blockType)(i, e));
    var n;
    return (
      b(e) ||
        (i.match(l("listLayout"), !1)
          ? (n = r.list)
          : i.match(l("drawTable"), !1)
            ? (n = r.table)
            : i.match(l("linkDefinition"), !1)
              ? (n = r.linkDefinition)
              : i.match(l("definitionList"))
                ? (n = r.definitionList)
                : i.match(l("html"), !1) && (n = r.html)),
      (e.mode = n || r.text)(i, e)
    );
  },
  blockType: function (i, e) {
    var n, o;
    if (((e.layoutType = null), (n = i.match(l("type"))))) o = n[0];
    else return (e.mode = r.text)(i, e);
    return (
      (n = o.match(l("header")))
        ? ((e.layoutType = "header"), (e.header = parseInt(n[0][1])))
        : o.match(l("bq"))
          ? (e.layoutType = "quote")
          : o.match(l("bc"))
            ? (e.layoutType = "code")
            : o.match(l("foot"))
              ? (e.layoutType = "footnote")
              : o.match(l("notextile"))
                ? (e.layoutType = "notextile")
                : o.match(l("pre"))
                  ? (e.layoutType = "pre")
                  : o.match(l("div"))
                    ? (e.layoutType = "div")
                    : o.match(l("table")) && (e.layoutType = "table"),
      (e.mode = r.attributes),
      u(e)
    );
  },
  ...
  list: function (i, e) {
    ...
    return (
      o ? (o === 1 ? (e.layoutType = "list2") : (e.layoutType = "list3")) : (e.layoutType = "list1"),
      (e.mode = r.attributes),
      u(e)
    );
  },
  ...
  definitionList: function (i, e) {
    return (
      i.match(l("definitionList")),
      (e.layoutType = "definitionList"),
      i.match(/\s*$/) ? (e.spanningLayout = !0) : (e.mode = r.attributes),
      u(e)
    );
  },
  html: function (i) {
