...
function s(t, e) {
  ...
  if (t.eat(/^;/)) return (t.skipToEnd(), ["space", "comment"]);
  if (t.eat(/^[#'@^`~]/)) return [null, "meta"];
  var r = t.match(k),
    n = r && r[0];
  return n
    ? n === "comment" && e.lastToken === "("
      ? (e.tokenize = w)(t, e)
      : a(n, f) || n.charAt(0) === ":"
        ? ["symbol", "atom"]
        : a(n, m) || a(n, h)
...
const q = {
  name: "clojure",
  startState: function () {
    return { ctx: { prev: null, start: 0, indentTo: 0 }, lastToken: null, tokenize: s };
  },
  token: function (t, e) {
    ...
    return (
      n !== "space" &&
        (e.lastToken === "(" && e.ctx.indentTo === null
          ? n === "symbol" && a(c, y)
            ? (e.ctx.indentTo = e.ctx.start + t.indentUnit)
            : (e.ctx.indentTo = "next")
          : e.ctx.indentTo === "next" && (e.ctx.indentTo = t.column()),
        (e.lastToken = c)),
      n === "open"
        ? (e.ctx = { prev: e.ctx, start: t.column(), indentTo: null })
        : n === "close" && (e.ctx = e.ctx.prev || e.ctx),
      i
    );
  },
  indent: function (t) {
