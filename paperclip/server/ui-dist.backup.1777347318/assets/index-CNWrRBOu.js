import { L as v, E as i, C as _ } from "./index-BCpNKgwK.js";
import { s as W, t as e, a as g, L as x, f as p, j as V, i as U, c as E } from "./index-BSvjNLKU.js";
import { defineCSSCompletionSource as N } from "./index-BgNo7vns.js";
const j = 168,
  X = 169,
  C = 170,
  I = 1,
...
const nO = new i(
    (O, $) => {
      if ($.dialectEnabled(c)) {
        let Q;
        if (O.next < 0 && $.canShift(Z)) O.acceptToken(Z);
        else if (((Q = O.peek(-1)) == r || Q < 0) && $.canShift(T)) {
          let t = 0;
          for (; O.next != r && R.includes(O.next); ) (O.advance(), t++);
          O.next == r || h(O) ? O.acceptToken(T, -t) : t && O.acceptToken(s);
        } else if (O.next == r) O.acceptToken(A, 1);
        ...
        let Q = 0;
        for (; R.includes(O.next); ) (O.advance(), Q++);
        Q && O.acceptToken(s);
      }
    },
    { contextual: !0 }
        ...
        if (Q > -1) {
          ...
          for (O.advance(); O.next >= 0; )
            if (O.next == r) {
              O.advance();
              let n = 0;
              for (; O.next != r && R.includes(O.next); ) (n++, O.advance());
              if (n < Q) {
                a = -n - 1;
                break;
              }
  ...
  iO = new i((O, $) => {
    ...
    if (O.peek(-1) == r) {
      let a = 0;
      for (; O.next != r && R.includes(O.next); ) (O.advance(), a++);
  ...
  oO = new i((O, $) => {
    for (let Q = !1, t = 0, a = 0; ; a++) {
      ...
      if (o(n) || n == l || n == m || (Q && y(n)))
        ...
        Q && O.acceptToken(t == 2 && $.canShift(Y) ? Y : $.canShift(z) ? z : n == J ? L : F);
        break;
      }
    }
  }),
  lO = new i((O) => {
    if (O.next == QO) {
      for (O.advance(); o(O.next) || O.next == l || O.next == m || y(O.next); ) O.advance();
  ...
  dO = new i((O) => {
    if (R.includes(O.peek(-1))) {
      let { next: $ } = O;
      (o($) || $ == m || $ == f || $ == $O || $ == OO || ($ == M && o(O.peek(1))) || $ == l || $ == aO || $ == S) &&
        O.acceptToken(C);
    }
  }),
  SO = new i((O) => {
...
const cO = new b(null, 0),
  ...
  wO = P.configure({
    dialect: "indented",
    props: [
      U.add({ "Block RuleSet": (O) => O.baseIndent + O.unit }),
      p.add({ Block: (O) => ({ from: O.from, to: O.to }) }),
    ],
  }),
  YO = N((O) => O.name == "VariableName" || O.name == "SassVariableName");
function GO(O) {
  return new g(O != null && O.indented ? wO : P, P.data.of({ autocomplete: YO }));
}
export { GO as sass, YO as sassCompletionSource, P as sassLanguage };
