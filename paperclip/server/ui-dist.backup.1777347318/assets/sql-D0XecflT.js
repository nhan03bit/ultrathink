function s(a) {
  var c = a.client || {},
    h = a.atoms || { false: !0, true: !0, null: !0 },
    p = a.builtin || e(z),
    S = a.keywords || e(d),
    _ = a.operatorChars || /^[*+\-%<>!=&|~^\/]/,
    o = a.support || {},
    y = a.hooks || {},
    C = a.dateSQL || { date: !0, time: !0, timestamp: !0 },
    Q = a.backslashStringEscapes !== !1,
    j = a.brackets || /^[\{}\(\)\[\]]/,
    v = a.punctuation || /^[;.,:]/;
  function g(t, i) {
    var r = t.next();
    ...
    if (
      o.hexNumber &&
      ((r == "0" && t.match(/^[xX][0-9a-fA-F]+/)) || ((r == "x" || r == "X") && t.match(/^'[0-9a-fA-F]*'/)))
    )
      return "number";
    ...
    if (r == "?" && (t.eatSpace() || t.eol() || t.eat(";"))) return "macroName";
    if (r == "'" || (r == '"' && o.doubleQuote)) return ((i.tokenize = x(r)), i.tokenize(t, i));
    if (
      ((o.nCharCast && (r == "n" || r == "N")) || (o.charsetCast && r == "_" && t.match(/[a-z][a-z0-9]*/i))) &&
      (t.peek() == "'" || t.peek() == '"')
    )
      return "keyword";
    if (o.escapeConstant && (r == "e" || r == "E") && (t.peek() == "'" || (t.peek() == '"' && o.doubleQuote)))
      return (
        (i.tokenize = function (m, k) {
          return (k.tokenize = x(m.next(), !0))(m, k);
        }),
        "keyword"
      );
    if (o.commentSlashSlash && r == "/" && t.eat("/")) return (t.skipToEnd(), "comment");
    if ((o.commentHash && r == "#") || (r == "-" && t.eat("-") && (!o.commentSpaceRequired || t.eat(" "))))
      return (t.skipToEnd(), "comment");
    if (r == "/" && t.eat("*")) return ((i.tokenize = b(1)), i.tokenize(t, i));
    if (r == ".") {
      if (o.zerolessFloat && t.match(/^(?:\d+(?:e[+-]?\d+)?)/i)) return "number";
      if (t.match(/^\.+/)) return null;
      if (o.ODBCdotTable && t.match(/^[\w\d_$#]+/)) return "type";
    } else {
      if (_.test(r)) return (t.eatWhile(_), "operator");
      if (j.test(r)) return "bracket";
      if (v.test(r)) return (t.eatWhile(v), "punctuation");
      if (
        r == "{" &&
        (t.match(/^( )*(d|D|t|T|ts|TS)( )*'[^']*'( )*}/) || t.match(/^( )*(d|D|t|T|ts|TS)( )*"[^"]*"( )*}/))
      )
        return "number";
      t.eatWhile(/^[_\w\d]/);
      var l = t.current().toLowerCase();
      return C.hasOwnProperty(l) && (t.match(/^( )+'[^']*'/) || t.match(/^( )+"[^"]*"/))
        ? "number"
        : h.hasOwnProperty(l)
          ? "atom"
          : p.hasOwnProperty(l)
  ...
  function w(t, i, r) {
    i.context = { prev: i.context, indent: t.indentation(), col: t.column(), type: r };
  }
  function L(t) {
...
function f(a) {
  for (var c; (c = a.next()) != null; ) if (c == "`" && !a.eat("`")) return "string.special";
...
function u(a) {
  return (
    a.eat("@") && (a.match("session."), a.match("local."), a.match("global.")),
    a.eat("'")
      ? (a.match(/^.*'/), "string.special")
      : a.eat('"')
        ? (a.match(/^.*"/), "string.special")
        : a.eat("`")
          ? (a.match(/^.*`/), "string.special")
...
function e(a) {
  for (var c = {}, h = a.split(" "), p = 0; p < h.length; ++p) c[h[p]] = !0;
  return c;
}
var z =
