...
const d = {
  ...
  token: function (r, e) {
    ...
    if (r.match(p)) return ((e.controlFlow = !0), "keyword");
    if (e.controlFlow) {
      if (r.match(u)) return "operator";
      if (r.match(/^(\d+)/)) return "number";
      r.eol() && (e.controlFlow = !1);
    }
    if (r.match(i)) return (r.eol() && (e.controlFlow = !1), "number");
    if (r.match(/^%[\w]+/)) return (r.match("(") && (e.macroParameters = !0), "keyword");
    if (e.macroParameters) {
      if (r.match(/^\d+/)) return "number";
      if (r.match(")")) return ((e.macroParameters = !1), "keyword");
    }
    return r.match(/^%\{\??[\w \-\:\!]+\}/) ? (r.eol() && (e.controlFlow = !1), "def") : (r.next(), null);
  },
};
export { h as rpmChanges, d as rpmSpec };
