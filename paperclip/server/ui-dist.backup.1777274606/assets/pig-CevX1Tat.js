        ...
        : O == "/"
          ...
          : O == "-"
            ...
            : I.test(O)
              ? (e.eatWhile(I), "operator")
              : (e.eatWhile(/[\w\$_]/),
                n && n.propertyIsEnumerable(e.current().toUpperCase()) && !e.eat(")") && !e.eat(".")
                  ? "keyword"
                  : S && S.propertyIsEnumerable(e.current().toUpperCase())
                    ? "builtin"
                    : t && t.propertyIsEnumerable(e.current().toUpperCase())
                      ? "type"
                      : "variable");
}
const G = {
  name: "pig",
  startState: function () {
    return { tokenize: A, startOfLine: !0 };
  },
  token: function (e, T) {
