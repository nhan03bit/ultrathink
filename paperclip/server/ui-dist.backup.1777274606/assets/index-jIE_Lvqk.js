...
import { html as W } from "./index-BX_P5oBN.js";
import { L as V, E as c } from "./index-CPAOPyA6.js";
      ...
      if (r == 123) {
        ...
        for (;;) {
          ...
          if (Q == 32 || Q == 10) ++$;
            ...
            let t = Q == 101 && O.peek($ + 1) == 110 && O.peek($ + 2) == 100;
            O.acceptToken(t ? k : w, i);
            return;
          }
        }
      }
...
const C = g("endcomment", E, v),
  N = g("endraw", z, R),
  Y = new c((O) => {
    if (O.next == 35) {
      for (O.advance(); !(O.next == 10 || O.next < 0 || ((O.next == 37 || O.next == 125) && O.peek(1) == 125)); )
        O.advance();
      O.acceptToken(U);
    }
  }),
  ...
  A = V.deserialize({
    ...
    repeatNodeCount: 11,
    tokenData:
      ")e~RmXY!|YZ!|]^!|pq!|qr#_rs#juv$[wx$gxy%Syz%X{|%^|}&x}!O&}!O!P'Z!Q![&g![!]'k!^!_'p!_!`'x!`!a'p!c!}(Q!}#O(y#P#Q)O#R#S(Q#T#o(Q#p#q)T#q#r)Y%W;'S(Q;'S;:j(s<%lO(Q~#RS%O~XY!|YZ!|]^!|pq!|~#bP!_!`#e~#jOb~~#mUOY#jZr#jrs$Ps;'S#j;'S;=`$U<%lO#j~$UOo~~$XP;=`<%l#j~$_P#q#r$b~$gOx~~$jUOY$gZw$gwx$Px;'S$g;'S;=`$|<%lO$g~%PP;=`<%l$g~%XOg~~%^Of~P%aQ!O!P%g!Q![&gP%jP!Q![%mP%rRpP!Q![%m!g!h%{#X#Y%{P&OR{|&X}!O&X!Q![&_P&[P!Q![&_P&dPpP!Q![&_P&lSpP!O!P%g!Q![&g!g!h%{#X#Y%{~&}Ou~~'QRuv$[!O!P%g!Q![&g~'`Q]S!O!P'f!Q![%m~'kOi~~'pOt~~'uPb~!_!`#e~'}Pe~!_!`#e_(ZW^WwQ%RT}!O(Q!Q![(Q!c!}(Q#R#S(Q#T#o(Q%W;'S(Q;'S;:j(s<%lO(Q_(vP;=`<%l(Q~)OO%T~~)TO%S~~)YOr~~)]P#q#r)`~)eOX~",
    tokenizers: [F, N, C, Y, 0, 1, 2, 3],
    topRules: { Template: [0, 7] },
    specialized: [
      { term: 187, get: (O) => I[O] || -1 },
      { term: 39, get: (O) => L[O] || -1 },
    ],
    tokenPrec: 0,
  });
...
const q = l(
  ...
  T = l("empty forloop tablerowloop in with as", "keyword"),
  B = l("first index index0 last length rindex", "property"),
  H = l("col col0 col_first col_last first index index0 last length rindex rindex0 row", "property");
function M(O) {
  var a;
  let { state: e, pos: r } = O,
    $ = y(e).resolveInner(r, -1).enterUnfinishedNodesBefore(r),
...
function J(O, a, e, r) {
  ...
  for (;;) {
    ...
    if (!i) return [];
    if (i.name == "forloop") return $.length ? [] : B;
    if (i.name == "tablerowloop") return $.length ? [] : H;
    if (i.name == "VariableName") {
      $.unshift(O.sliceDoc(i.from, i.to));
      break;
    } else if (i.name == "MemberExpression") {
      let Q = i.getChild("PropertyName");
      (Q && $.unshift(O.sliceDoc(Q.from, Q.to)), (a = i));
    } else return [];
  }
  return r ? r($, O, e) : [];
}
function K(O = {}) {
  let a = O.filters ? O.filters.concat(q) : q,
    e = O.tags ? O.tags.concat(m) : m,
    r = O.variables ? O.variables.concat(T) : T,
    { properties: $ } = O;
  return (i) => {
    var Q;
    let t = M(i);
    if (!t) return null;
    let o = (Q = t.from) !== null && Q !== void 0 ? Q : t.node ? t.node.from : i.pos,
      s;
    return (
      t.type == "filter"
        ? (s = a)
        : t.type == "tag"
          ...
          : t.type == "expression"
            ? (s = r)
            : (s = J(i.state, t.target, i, $)),
      s.length ? { options: s, from: o, validFor: /^[\w\u00c0-\uffff]*$/ } : null
    );
  };
}
const OO = u.inputHandler.of((O, a, e, r) =>
  r != "%" || a != e || O.state.doc.sliceString(a - 1, e + 1) != "{}"
    ? !1
    : (O.dispatch(
        O.state.changeByRange(($) => ({
          changes: { from: $.from, to: $.to, insert: "%%" },
          range: x.cursor($.from + 1),
        })),
...
const $O = X.define({
    ...
    parser: A.configure({
      props: [
        b({
          "cycle comment endcomment raw endraw echo increment decrement liquid in with as": n.keyword,
          "empty forloop tablerowloop": n.atom,
          "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue":
            n.controlKeyword,
          "assign capture endcapture": n.definitionKeyword,
          contains: n.operatorKeyword,
          "render include": n.moduleKeyword,
          VariableName: n.variableName,
          TagName: n.tagName,
          FilterName: n.function(n.variableName),
          PropertyName: n.propertyName,
          CompareOp: n.compareOperator,
          AssignOp: n.definitionOperator,
          LogicOp: n.logicOperator,
          NumberLiteral: n.number,
          StringLiteral: n.string,
          BooleanLiteral: n.bool,
          InlineComment: n.lineComment,
          CommentText: n.blockComment,
          "{% %} {{ }}": n.brace,
          "[ ]": n.bracket,
          "( )": n.paren,
          ".": n.derefOperator,
        ...
        Z.add({
          Tag: j({ closing: "%}" }),
          "UnlessDirective ForDirective TablerowDirective CaptureDirective": d(/^\s*(\{%-?\s*)?end\w/),
          IfDirective: d(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/),
          CaseDirective: d(/^\s*(\{%-?\s*)?(endcase|when)\b/),
        }),
        G.add({
          "UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(
            O
          ) {
            let a = O.firstChild,
              e = O.lastChild;
...
function f(O) {
  return $O.configure(
    {
      wrap: h((a) =>
        a.type.isTop ? { parser: O.parser, overlay: (e) => e.name == "Text" || e.name == "RawText" } : null
      ),
    },
    "liquid"
  );
}
const aO = f(P.language);
function tO(O = {}) {
  let a = O.base || P,
    e = a.language == P.language ? aO : f(a.language);
  return new p(e, [
    a.support,
...
export { OO as closePercentBrace, tO as liquid, K as liquidCompletionSource, aO as liquidLanguage };
