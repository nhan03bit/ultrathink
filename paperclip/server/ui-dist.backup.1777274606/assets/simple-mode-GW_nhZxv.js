  ...
  for (var d in n)
    if (d != e && n.hasOwnProperty(d))
      for (var u = (i[d] = []), r = n[d], t = 0; t < r.length; t++) {
        var f = r[t];
        (u.push(new s(f, n)), (f.indent || f.dedent) && (p = !0));
      }
  return {
    name: e.name,
    startState: function () {
...
function h(n, i) {
  ...
  var e = "";
  return (
    n instanceof RegExp ? (n.ignoreCase && (e = "i"), n.unicode && (e += "u"), (n = n.source)) : (n = String(n)),
    new RegExp("^(?:" + n + ")", e)
  );
}
function a(n) {
...
function l(n) {
  return function (i, e) {
    ...
    for (var d = n[e.state], u = 0; u < d.length; u++) {
      var r = d[u],
        t = (!r.data.sol || i.sol()) && i.match(r.regex);
      if (t) {
        (r.data.next
          ? (e.state = r.data.next)
          : r.data.push
            ? ((e.stack || (e.stack = [])).push(e.state), (e.state = r.data.push))
            : r.data.pop && e.stack && e.stack.length && (e.state = e.stack.pop()),
          r.data.indent && e.indent.push(i.indentation() + i.indentUnit),
          r.data.dedent && e.indent.pop());
        var f = r.token;
