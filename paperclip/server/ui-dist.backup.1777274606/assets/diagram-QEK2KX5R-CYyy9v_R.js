...
import { p as V } from "./chunk-4BX2VUAB-Cb4O8Gqh.js";
import { p as W } from "./treemap-GDKQZRPO-qHAS-c0V.js";
...
var h = { showLegend: !0, ticks: 5, max: null, min: 0, graticule: "circle" },
  w = { axes: [], curves: [], options: h },
  m = structuredClone(w),
  B = z.radar,
  j = l(() => y({ ...B, ...C().radar }), "getConfig"),
  b = l(() => m.axes, "getAxes"),
  q = l(() => m.curves, "getCurves"),
  K = l(() => m.options, "getOptions"),
  N = l((a) => {
    m.axes = a.map((t) => ({ name: t.name, label: t.label ?? t.name }));
  }, "setAxes"),
  U = l((a) => {
    m.curves = a.map((t) => ({ name: t.name, label: t.label ?? t.name, entries: X(t.entries) }));
  }, "setCurves"),
  X = l((a) => {
  ...
  e.forEach((d, u) => {
    ...
    o === "circle"
      ...
      : o === "polygon" &&
        a
          .append("polygon")
          .attr("points", p.map((g) => `${g.x},${g.y}`).join(" "))
          .attr("class", `radarCurve-${u}`);
  });
}
l(M, "drawCurves");
function A(a, t, e, r) {
...
function L(a, t) {
  ...
  for (let s = 0; s < e; s++) {
    const o = a[(s - 1 + e) % e],
      ...
      u = { x: n.x - (c.x - i.x) * t, y: n.y - (c.y - i.y) * t };
...
var st = { draw: tt },
  ...
  ht = { parser: Q, db: $, renderer: st, styles: it };
