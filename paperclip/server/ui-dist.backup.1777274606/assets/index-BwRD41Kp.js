import { L as M, E as X, a as w, C as E } from "./index-CPAOPyA6.js";
import {
  s as A,
  t as i,
  E as C,
...
const rO = 316,
  ...
  fO = new X(
    (O, a) => {
      let { next: Q } = O;
      (Q == R || Q == -1 || a.context) && O.acceptToken(ZO);
    },
    ...
    (O, a) => {
      ...
      if (Q == gO || Q == mO) {
        if ((O.advance(), Q == O.next)) {
          O.advance();
          let e = !a.context && a.canShift(b);
          O.acceptToken(e ? b : tO);
        }
      } else
...
const bO = new X((O, a) => {
  ...
  zO = A({
    ...
    "NewExpression/VariableName": i.className,
    ...
    "is keyof unique infer asserts": i.operatorKeyword,
  ...
  RO = M.deserialize({
    ...
    specialized: [
      { term: 327, get: (O) => jO[O] || -1 },
      { term: 343, get: (O) => qO[O] || -1 },
      { term: 95, get: (O) => hO[O] || -1 },
    ],
    tokenPrec: 15201,
  }),
  W = [
    Z("function ${name}(${params}) {\n	${}\n}", { label: "function", detail: "definition", type: "keyword" }),
    Z("for (let ${index} = 0; ${index} < ${bound}; ${index}++) {\n	${}\n}", {
      label: "for",
      detail: "loop",
      type: "keyword",
    }),
    Z("for (let ${name} of ${collection}) {\n	${}\n}", { label: "for", detail: "of loop", type: "keyword" }),
  ...
  return (
    a.cursor(eO.IncludeAnonymous).iterate(($) => {
      if (r) r = !1;
        ...
        for (let l of G(O, $.node)) e.push(l);
        return !1;
      }
    }),
    z.set(a, e),
...
function VO(O) {
  ...
  let e = [];
  for (let r = a; r; r = r.parent) v.has(r.name) && (e = e.concat(G(O.state.doc, r)));
  return { options: e, from: Q ? a.from : O.pos, validFor: m };
}
function T(O, a, Q) {
  var e;
  let r = [];
  for (;;) {
    ...
    if (
      ...
      ((e = $ = t.lastChild) === null || e === void 0 ? void 0 : e.name) == "PropertyName"
    )
      (r.push(O($)), (a = t));
    else return null;
  }
...
function UO(O) {
  let a = (e) => O.state.doc.sliceString(e.from, e.to),
...
function LO(O, a) {
  ...
  for (let r = 0; ; r++) {
    for (let $ of (Object.getOwnPropertyNames || Object.keys)(O)) {
      if (!/^[a-zA-Z_$\xaa-\uffdc][\w$\xaa-\uffdc]*$/.test($) || e.has($)) continue;
      e.add($);
      let l;
      try {
        l = O[$];
...
function FO(O) {
  ...
  return (Q) => {
    ...
    if (!e) return null;
    let r = O;
    for (let $ of e.path) if (((r = r[$]), !r)) return null;
    let t = a.get(r);
    return (t || a.set(r, (t = LO(r, !e.path.length))), { from: Q.pos - e.name.length, options: t, validFor: m });
  };
}
const P = K.define({
    name: "javascript",
  ...
  EO = P.configure({ dialect: "jsx", props: [h.add((O) => (O.isTop ? [V] : void 0))] }),
  AO = P.configure({ dialect: "jsx ts", props: [h.add((O) => (O.isTop ? [V] : void 0))] }, "typescript");
    ...
    let t = r(),
      ...
      l = $.changeByRange((o) => {
        ...
        if (
          ...
          !($.doc.sliceString(n - 1, n) != e || (s.name == "JSXAttributeValue" && s.to > n)))
        ) {
          if (e == ">" && s.name == "JSXFragmentTag") return { range: o, changes: { from: n, insert: "</>" } };
          if (e == "/" && s.name == "JSXStartCloseTag") {
            let c = s.parent,
              u = c.parent;
            if (
              u &&
              c.from == n - 2 &&
              ((g = j($.doc, u.firstChild, n)) ||
            ...
            let c = IO(s);
            if (c && c.name == "JSXOpenTag" && !/^\/?>|^<\//.test($.doc.sliceString(n, n + 2)) && (g = j($.doc, c, n)))
              return { range: o, changes: { from: n, insert: `</${g}>` } };
          }
        }
        return { range: o };
      });
  ...
  return (
    a ||
      ...
      O.getRules().forEach((Q, e) => {
        var r;
        !((r = Q.meta.docs) === null || r === void 0) && r.recommended && (a.rules[e] = 2);
      })),
    (Q) => {
      let { state: e } = Q,
        r = [];
      for (let { from: t, to: $ } of P.findRegions(e)) {
        let l = e.doc.lineAt(t),
          o = { line: l.number - 1, col: t - l.from, pos: t };
        for (let p of O.verify(e.sliceDoc(t, $), a)) r.push(NO(p, e.doc, o));
      }
      return r;
    }
  );
...
function NO(O, a, Q) {
  let e = q(O.line, O.column, a, Q),
    r = {
      from: e,
      to: O.endLine != null && O.endColumn != 1 ? q(O.endLine, O.endColumn, a, Q) : e,
      message: O.message,
    ...
    r.actions = [
      {
        name: "fix",
        apply(p, n) {
          p.dispatch({ changes: { from: n + l, to: n + o, insert: $ }, scrollIntoView: !0 });
        },
      },
    ];
  }
