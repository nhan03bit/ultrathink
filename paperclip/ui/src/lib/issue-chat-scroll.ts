export type IssueChatScrollTarget = { type: "element"; element: HTMLElement } | { type: "window" };

export interface ComposerViewportSnapshot {
  composerViewportTop: number;
}

export function resolveIssueChatScrollTarget(doc: Document = document, win: Window = window): IssueChatScrollTarget {
...
export function captureComposerViewportSnapshot(composerElement: HTMLElement | null): ComposerViewportSnapshot | null {
...
export function shouldPreserveComposerViewport(composerElement: HTMLElement | null, doc: Document = document) {
