...
export function useCompanyOrder({ companies, userId }: UseCompanyOrderParams) {
  ...
  const queryKey = useMemo(() => queryKeys.sidebarPreferences.companyOrder(userId ?? "__anon__"), [userId]);
  ...
  const orderedCompanies = useMemo(() => sortCompaniesByOrder(companies, orderedIds), [companies, orderedIds]);
