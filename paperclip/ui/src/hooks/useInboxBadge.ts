...
export function useInboxDismissals(companyId: string | null | undefined) {
  ...
  const queryKey = companyId ? queryKeys.inboxDismissals(companyId) : (["inbox-dismissals", "__disabled__"] as const);
  ...
  const dismissedAtByKey = useMemo(() => buildInboxDismissedAtByKey(dismissals), [dismissals]);
