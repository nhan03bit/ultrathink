...
export function usePaperclipIssueRuntime({ messages, isRunning, onSend, onCancel }: UsePaperclipIssueRuntimeOptions) {
  ...
  const adapter = useMemo<ExternalStoreAdapter<ThreadMessage>>(
    () => ({
      ...
      onNew: async (message) => {
        ...
        const reassignment =
          reassignmentRecord && ("assigneeAgentId" in reassignmentRecord || "assigneeUserId" in reassignmentRecord)
            ? {
                assigneeAgentId:
