...
describe("IssueDocumentsSection", () => {
  ...
  it("shows the restored document body immediately after a revision restore", async () => {
    ...
    queryClient.setQueryData(queryKeys.issues.documentRevisions(issue.id, "plan"), [
      createRevision({
        id: "revision-4",
        revisionNumber: 4,
        body: "",
    ...
    const restoreButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("Restore this revision")
    );
  ...
  it("returns from a historical preview when the current revision only exists in derived state", async () => {
    ...
    const currentRevisionButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("rev 4")
    );
  ...
  it("returns from a historical preview when fetched history is newer than the document summary", async () => {
    ...
    const currentRevisionButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("rev 3")
    );
    expect(currentRevisionButton).toBeTruthy();
