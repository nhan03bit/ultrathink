...
describe("IssueProperties", () => {
  ...
  it("always exposes the add sub-issue action", async () => {
    ...
    const addButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("Add sub-issue")
    );
  ...
  it("allows setting and clearing a parent issue from the properties pane", async () => {
    ...
    const parentTrigger = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("No parent")
    );
    ...
    const candidateButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("PAP-2 Candidate parent")
    );
    ...
    const selectedParentTrigger = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("PAP-2 Candidate parent")
    );
    ...
    const clearParentButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("No parent")
    );
  ...
  it("shows a run review action after reviewers are configured and starts execution explicitly when clicked", async () => {
    ...
    const runReviewButton = Array.from(container.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("Run review now")
    );
