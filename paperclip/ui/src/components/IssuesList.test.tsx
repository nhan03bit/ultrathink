...
describe("IssuesList", () => {
  ...
  it("uses the supplied create defaults and label for sub-issue lists", async () => {
    ...
    await waitForAssertion(() => {
      const button = Array.from(container.querySelectorAll("button")).find((candidate) =>
        candidate.textContent?.includes("New Sub-issue")
      );
  ...
  it("shows routine-backed issues by default and hides them when the routine filter is toggled off", async () => {
    ...
    await waitForAssertion(() => {
      const toggle = Array.from(document.body.querySelectorAll("label")).find((label) =>
        label.textContent?.includes("Hide routine runs")
      );
