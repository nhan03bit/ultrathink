...
export function ImageGalleryModal({ images, initialIndex, open, onOpenChange }: ImageGalleryModalProps) {
  ...
  const handleBackdropClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (target.closest("button") || target.closest("a") || target === imageRef.current) return;
      onOpenChange(false);
    },
    [onOpenChange]
  );
