...
describe("buildWorkspaceRuntimeControlSections", () => {
  it("separates service and job commands while matching running services", () => {
    const sections = buildWorkspaceRuntimeControlSections({
      ...
      runtimeServices: [createRuntimeService({ id: "service-web", serviceName: "web", status: "running" })],
...
describe("WorkspaceRuntimeControls", () => {
  ...
  it("renders service and job actions distinctly", () => {
    ...
    act(() => {
      root.render(<WorkspaceRuntimeControls sections={sections} onAction={vi.fn()} />);
    });
  ...
  it("shows disabled actions when local command prerequisites are missing", () => {
    ...
    act(() => {
      root.render(
        <WorkspaceRuntimeControls sections={sections} disabledHint="Add a workspace path first." onAction={vi.fn()} />
      );
    });
  ...
  it("hides the disabled hint once services can already run", () => {
    const sections = buildWorkspaceRuntimeControlSections({
      runtimeConfig: {
        commands: [{ id: "web", name: "web", kind: "service", command: "pnpm dev" }],
      },
      runtimeServices: [createRuntimeService({ id: "service-web", serviceName: "web", status: "running" })],
      canStartServices: true,
    });
    ...
    act(() => {
      root.render(
        <WorkspaceRuntimeControls sections={sections} disabledHint="Add runtime settings first." onAction={vi.fn()} />
      );
    });
