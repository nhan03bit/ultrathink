...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={`${day}: ${total} issues`}>
              {total > 0 ? (
                ...
                >
                  {priorityOrder.map((p) =>
                    entry[p] > 0 ? <div key={p} style={{ flex: entry[p], backgroundColor: priorityColors[p] }} /> : null
                  )}
                </div>
              ) : (
                <div className="bg-muted/30 rounded-sm" style={{ height: 2 }} />
      ...
      <DateLabels days={days} />
      <ChartLegend
        items={priorityOrder.map((p) => ({ color: priorityColors[p], label: p.charAt(0).toUpperCase() + p.slice(1) }))}
      />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  const statusOrder = ["todo", "in_progress", "in_review", "done", "blocked", "cancelled", "backlog"].filter((s) =>
    allStatuses.has(s)
  );
  const maxValue = Math.max(...Array.from(grouped.values()).map((v) => Object.values(v).reduce((a, b) => a + b, 0)), 1);
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend
        items={statusOrder.map((s) => ({ color: statusColors[s] ?? "#6b7280", label: statusLabels[s] ?? s }))}
      />
    </div>
  );
}
        ...
        {days.map((day) => {
          ...
          return (
            <div
              ...
              title={`${day}: ${entry.total > 0 ? Math.round(rate * 100) : 0}% (${entry.succeeded}/${entry.total})`}
            >
              {entry.total > 0 ? (
