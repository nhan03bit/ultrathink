 ...
 */
function extractDateTerms(content) {
  const terms = [];
 ...
 */
export function enrichMemory(content, category, tags) {
  const words = content
    .toLowerCase()
    .replace(/[^a-z0-9\s\-_]/g, " ")
    .split(/\s+/)
    .filter((w) => w.length > 2);
  const enrichTerms = new Set();
 ...
 */
export function enrichQuery(query) {
  const terms = new Set();
  const words = query
    .toLowerCase()
    .replace(/[^a-z0-9\s\-_]/g, " ")
 ...
 */
export function expandQuerySynonyms(query) {
  const terms = new Set();
  const words = query
    .toLowerCase()
    .replace(/[^a-z0-9\s\-_]/g, " ")
