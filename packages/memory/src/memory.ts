...
// intent: Second Brain — 4-wing architecture (agent/user/knowledge/experience)
// status: done
// confidence: high
 ...
 */
export function passesQualityGate(content: string, layer?: Layer): string | null {
 ...
 */
function autoTitle(content: string, category: string): string {
 ...
 */
function inferStructure(input: CreateMemoryInput): { wing: Wing; hall: string; layer: Layer; token_estimate: number } {
...
async function autoLinkMemory(memoryId: string, content: string, scope?: string): Promise<void> {
  ...
  const [source] = (await sql`SELECT wing, hall FROM memories WHERE id = ${memoryId}`) as any[];
  const srcWing = source?.wing as string | undefined;
  const srcHall = source?.hall as string | undefined;

  for (const r of rows as any[]) {
    const relType = inferRelationType(srcWing, srcHall, r.wing as string, r.hall as string);
    await sql`
      INSERT INTO memory_relations (source_id, target_id, relation_type, strength)
      VALUES (${memoryId}, ${r.id as string}, ${relType}, ${Number(r.sim ?? 0.5)})
      ON CONFLICT DO NOTHING
    `;
  }
}
