...
const FRAMEWORKS = [
    ...
    },
    {
        ...
        instruction: "List all options as columns. Define criteria as rows with explicit weights (must sum to 100%). Score each option 1-5 per criterion. Multiply score × weight. Sum columns to get weighted totals. The option with highest weighted total wins — but surface the criteria weights for validation first.",
        keywords: [
            "compare",
            "comparison",
            "options",
...
export function isDecisionPrompt(prompt) {
    return DECISION_SIGNALS.some((r) => r.test(prompt));
}
export function buildDecisionContext(prompt) {
